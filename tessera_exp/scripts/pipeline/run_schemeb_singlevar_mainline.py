...
def render_summary_md(rows: list[dict], out_json: Path, variable: str) -> str:
    md = [
        ...
        "",
        "| value | tessera_f1 | tessera_em | tessera_r10 | dense_f1 | text_only_f1 | table_only_f1 | kg_only_f1 | multi_modal_f1 | tapas_enabled |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for r in rows:
        md.append(
            f"| {r['value']} | {r['tessera_f1']:.4f} | {r['tessera_em']:.4f} | {r['tessera_r10']:.4f} | {r['dense_f1']:.4f} | "
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run single-variable mainline sweeps for SchemeB (dense + tessera only)")
