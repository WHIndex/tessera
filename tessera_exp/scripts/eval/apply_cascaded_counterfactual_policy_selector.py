...
def main() -> int:
    ...
    parser.add_argument("--label", type=str, default="TESSERA-Cascade-CEPS")
        ...
        debug_rows.append(
            {
                ...
                "rankings": {
                    ...
                    "_base_tessera": base,
                    "_primary_tessera": primary_ranked,
                },
                "trace": {
                    "tessera_cascade_active": 1.0,
                    "tessera_cascade_primary_policy": primary_selection.policy,
                    "tessera_cascade_primary_switched": float(primary_selection.switched),
                    "tessera_cascade_primary_margin": float(primary_selection.margin),
                    "tessera_cascade_primary_scores": primary_selection.policy_scores,
                    "tessera_cascade_addon_policy": addon_selection.policy,
                    "tessera_cascade_addon_switched": float(addon_selection.switched),
                    "tessera_cascade_addon_margin": float(addon_selection.margin),
                    "tessera_cascade_addon_scores": addon_selection.policy_scores,
                    "tessera_cascade_addon_positive_probs": addon_selection.positive_probs,
                    "tessera_cascade_final_policy": final_policy,
                    "tessera_cascade_final_switched": float(final_changed),
                },
            }
        )
    ...
    if bool(args.include_primary):
        ...
        table_rows.insert(0, {"method": "primary_tessera", "label": "Primary CEPS", **primary_summary})
        details["methods"]["primary_tessera"] = primary_detail
