...
def compare(metrics: dict[str, dict[str, float]]) -> None:
    ...
    tessera = metrics.get("tessera_rag")
    submod = metrics.get("tessera_submod")
    ...
    if tessera:
        print(_fmt("TESSERA-RAG", tessera))
    if submod:
        print(_fmt("TESSERA-Submod", submod))

    print("-" * 60)
    if tessera:
        df1 = tessera["f1"] - baseline["f1"]
        dem = tessera["exact_match"] - baseline["exact_match"]
        dr10 = tessera["recall_at_10"] - baseline["recall_at_10"]
        print(f"TESSERA vs Dense   ΔF1={df1:+.4f}  ΔEM={dem:+.4f}  ΔR@10={dr10:+.4f}")
    if submod:
    ...
    if tessera and submod:
        df1 = submod["f1"] - tessera["f1"]
        dem = submod["exact_match"] - tessera["exact_match"]
        dr10 = submod["recall_at_10"] - tessera["recall_at_10"]
        print(f"Submod   vs TESSERA ΔF1={df1:+.4f}  ΔEM={dem:+.4f}  ΔR@10={dr10:+.4f}")
    print("=" * 60)
