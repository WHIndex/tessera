...
def compute_metrics(queries, ctx_docs_dense, ctx_docs_uni, doc_map):
    ...
    # 1. Modality coverage
    for method_name, ctx_docs in [("dense_concat", ctx_docs_dense), ("tessera_rag", ctx_docs_uni)]:
        mod_counts = Counter()
    ...
    # 4. Comparison: overlap between Dense and TESSERA contexts
    overlap_ratios = []
    for q in queries:
        qid = q["id"]
...
def main():
    ...
    ctx_docs_uni = load_context_docs(f"{results_dir}/context_docs_tessera_rag_test1286.jsonl")
    ...
    for method in ["dense_concat", "tessera_rag"]:
        label = "Dense-Concat" if method == "dense_concat" else "TESSERA-RAG"
    ...
    print("\n=== Context Overlap (Dense vs TESSERA) ===")
