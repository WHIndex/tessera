...
from tessera_exp.utils import ensure_dir, infer_modalities_from_relevant_chunks, read_json, write_json
...
def main() -> int:
    ...
    parser.add_argument("--tessera-pred-file", type=Path, default=None)
    ...
    uni_map = load_pred_map(args.tessera_pred_file)
    ...
    for rank, (score, qid, row, chunks, dense_pred, uni_pred, target_type) in enumerate(scored, start=1):
        ...
        new_row["tessera_prediction"] = uni_pred
        new_row["dense_em"] = exact_match(dense_pred, str(row.get("answer", "")))
        new_row["tessera_em"] = exact_match(uni_pred, str(row.get("answer", "")))
    ...
    summary = {
        ...
        "tessera_pred_file": str(args.tessera_pred_file) if args.tessera_pred_file is not None else None,
