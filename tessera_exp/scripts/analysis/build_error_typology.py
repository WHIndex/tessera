...
def infer_tessera_error_type(row: dict) -> str:
    ...
    return "other_tessera_errors"


def main() -> int:
    parser = argparse.ArgumentParser(description="Build reproducible error typology for TESSERA vs Dense predictions")
    parser.add_argument("--gold-file", type=Path, required=True)
    parser.add_argument("--dense-pred-file", type=Path, required=True)
    parser.add_argument("--tessera-pred-file", type=Path, required=True)
    ...
    uni_pred = load_prediction_jsonl(args.tessera_pred_file)

    ids = sorted(set(rows_by_id.keys()) & set(dense_pred.keys()) & set(uni_pred.keys()))
    if not ids:
        raise SystemExit("No overlapping ids among gold/dense/tessera predictions")
    ...
    for qid in ids:
        ...
        if not uni_ok:
            err_type = infer_tessera_error_type(row)
    ...
    out = {
        "meta": {
            ...
            "tessera_pred_total": len(uni_pred),
            ...
            "error_type_rule": {
                ...
                "other_tessera_errors": "none of the above",
            },
        },
        ...
        "tessera_error_total": int(uni_error_total),
        "tessera_error_type_counts": dict(uni_error_type_counts),
        "tessera_error_type_ratios": {
    ...
    md = [
        "# TESSERA Error Typology",
        "",
        f"- evaluated: {out['meta']['evaluated']}",
        f"- coverage: {out['meta']['coverage']:.4f}",
        f"- tessera_error_total: {out['tessera_error_total']}",
        "",
        "## Outcome Summary",
        "",
        "| Outcome | Count | Ratio |",
    ...
    md.extend(
        [
            "",
            "## TESSERA Error Type Breakdown",
            "",
            "| Error Type | Count | Ratio (among TESSERA errors) |",
            "|---|---:|---:|",
        ]
    )

    for k in ["cross_modal_conflict", "numeric_reasoning", "alias_disambiguation", "other_tessera_errors"]:
        md.append(
            f"| {k} | {out['tessera_error_type_counts'].get(k, 0)} | {out['tessera_error_type_ratios'].get(k, 0.0):.4f} |"
        )
    ...
    for k in ["cross_modal_conflict", "numeric_reasoning", "alias_disambiguation", "other_tessera_errors"]:
        ...
        for ex in examples:
            ...
            md.append(f"  - tessera: {ex['tessera_prediction']}")
