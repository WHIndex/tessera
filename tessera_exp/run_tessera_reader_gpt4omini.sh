#!/usr/bin/env bash
# GPT-4o-mini reader evaluation from saved TESSERA rankings.
set -euo pipefail
...
OUT_DIR="${OUT_DIR:-${ROOT_DIR}/artifacts/results/${RUN_ID}_reader_gpt4omini_tessera}"
...
"${PYTHON_BIN}" scripts/eval/run_reader_from_rankings.py \
  ...
  --openai-timeout "${OPENAI_TIMEOUT:-180}" \
  --openai-max-retries "${OPENAI_MAX_RETRIES:-5}" \
  --openai-max-tokens "${OPENAI_MAX_TOKENS:-64}" \
  ${OPENAI_FAIL_SOFT:+--openai-fail-soft}

echo "[DONE] TESSERA GPT-4o-mini reader: ${OUT_DIR}"
