...
@dataclass(frozen=True)
class TESSERARetrievalAgentConfig:
...
@dataclass(frozen=True)
class TESSERAMoERetrievalConfig:
...
@dataclass(frozen=True)
class TESSERATableNumberAgentConfig:
...
def build_prf_token_weights(
    *,
    ...
    config: TESSERAMoERetrievalConfig,
) -> dict[str, float]:
...
def select_tessera_table_number_answer(
    *,
    query: str,
    current_answer: str,
    contexts: list[str],
    ...
    config: TESSERATableNumberAgentConfig | None = None,
) -> tuple[str, TableNumberAgentTrace]:
    """Extract a table/number-backed short answer for weak TESSERA reader outputs."""

    cfg = config or TESSERATableNumberAgentConfig()
...
def select_tessera_policy_context(
    *,
    query: str,
    ...
    config: TESSERAPolicyConfig | None = None,
) -> tuple[list[int], PolicyTrace]:
    cfg = config or TESSERAPolicyConfig()
    topk = max(1, int(k))
...
def select_tessera_retry_context(
    *,
    query: str,
    current_answer: str,
    base_ctx_idxs: list[int],
    ...
    upo_concept: str,
    source_bucket_fn: Callable[[str], str],
    config: TESSERARetryAgentConfig | None = None,
) -> tuple[list[int], RetryAgentTrace]:
    cfg = config or TESSERARetryAgentConfig()
    topk = max(1, int(k))
...
def rerank_tessera_retrieval(
    *,
    ...
    upo_concept: str,
    source_bucket_fn: Callable[[str], str],
    config: TESSERARetrievalAgentConfig | None = None,
) -> tuple[list[int], RetrievalAgentTrace]:
    """Multi-agent TESSERA retrieval reranker.

    The agents are lightweight scoring views rather than LLM calls: the base
    agent preserves TESSERA rank evidence, dense/sparse agents add recall,
    and target/coverage/diversity agents improve the top-k evidence set.
    """
    cfg = config or TESSERARetrievalAgentConfig()
    topk = max(1, int(k))
    profile = build_query_evidence_profile(
        query=query,
        router_prob=router_prob,
...
def rerank_tessera_moe_retrieval(
    *,
    query: str,
    current_ranked_idxs: list[int],
    candidate_idxs: Sequence[int],
    ...
    upo_concept: str,
    source_bucket_fn: Callable[[str], str],
    config: TESSERAMoERetrievalConfig | None = None,
) -> tuple[list[int], MoERetrievalTrace]:
    cfg = config or TESSERAMoERetrievalConfig()
    topk = max(1, int(k))
    profile = build_query_evidence_profile(
        query=query,
        router_prob=router_prob,
