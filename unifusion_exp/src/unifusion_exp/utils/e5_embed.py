...
class NVEmbedModel:
    def __init__(self, model):
        self.model = model
        self._unifusion_backend = "nvembed"
...
def _embedding_backend() -> str:
    return os.environ.get("UNIFUSION_EMBED_BACKEND", "hf").strip().lower()
...
def load_e5(model_dir: str, device: str | None = None):
    ...
    trust_remote = backend in {"nvembed", "nv-embed", "nv_embed"} or os.environ.get(
        "UNIFUSION_HF_TRUST_REMOTE_CODE", "0"
    ).strip() in {"1", "true", "yes"}
    ...
    device_map = os.environ.get("UNIFUSION_NV_DEVICE_MAP", "").strip()
    ...
    if backend in {"nvembed", "nv-embed", "nv_embed"}:
        if hasattr(model, "tokenizer"):
            try:
                model.tokenizer.padding_side = os.environ.get("UNIFUSION_NV_PADDING_SIDE", "right")
            except Exception:
                pass
...
def encode_texts(
    ...
    if getattr(model, "_unifusion_backend", "") == "nvembed":
        vecs = []
