...
TRACE_KEYS = [
    "unifusion_ser_changed_count",
    "unifusion_ser_topk_overlap",
    "unifusion_source_evidence_changed_count",
    "unifusion_source_evidence_topk_overlap",
    "unifusion_source_evidence_budget_changed_count",
    "unifusion_source_evidence_budget_tail_selected",
    "unifusion_source_evidence_budget_sibling_selected",
    "unifusion_source_evidence_budget_source_quota_selected",
    "unifusion_source_evidence_budget_reference_selected",
    "unifusion_source_head_top1_changed",
    "unifusion_gee_expand_triggered",
    "unifusion_gee_expand_graph_added",
]
    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if not line:
                continue
            row = json.loads(line)
            ranking = row.get("rankings", {}).get("unifusion_rag", [])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze query-level retrieval deltas between two UniFusion runs.")
