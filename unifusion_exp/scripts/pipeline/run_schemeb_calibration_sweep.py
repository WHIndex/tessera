...
def render_summary_md(rows: list[dict], out_json: Path) -> str:
    md = [
        ...
        "",
        "| mode | unifusion_f1 | unifusion_em | unifusion_r10 | dense_f1 | tapas_enabled | tapas_query_ready_mean | heavy_table_w_mean | heavy_kg_w_mean | heavy_token_late_w_mean |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for r in rows:
        md.append(
            f"| {r['mode']} | {r['unifusion_f1']:.4f} | {r['unifusion_em']:.4f} | {r['unifusion_r10']:.4f} | {r['dense_f1']:.4f} | "
