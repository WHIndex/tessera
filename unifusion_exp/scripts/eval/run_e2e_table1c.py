...
mod = importlib.import_module("unifusion_exp.utils.e5_embed")
load_e5 = mod.load_e5
encode_texts = mod.encode_texts
metrics_mod = importlib.import_module("unifusion_exp.e2e.metrics")
...
report_mod = importlib.import_module("unifusion_exp.e2e.reporting")
build_table1c_markdown = report_mod.build_table1c_markdown

methods_mod = importlib.import_module("unifusion_exp.e2e.methods")
...
baseline_mod = importlib.import_module("unifusion_exp.e2e.baselines")
...
objectives_mod = importlib.import_module("unifusion_exp.e2e.objectives")
...
controller_mod = importlib.import_module("unifusion_exp.e2e.controller")
...
try:
    from unifusion_exp.e2e.submodular_packing import submodular_t2g_packer
except Exception:
    submodular_t2g_packer = None  # type: ignore[misc,assignment]

try:
    policy_mod = importlib.import_module("unifusion_exp.e2e.unifusion_policy")
    UnifusionPolicyConfig = policy_mod.UnifusionPolicyConfig
    select_unifusion_policy_context = policy_mod.select_unifusion_policy_context
    UnifusionRetrievalAgentConfig = policy_mod.UnifusionRetrievalAgentConfig
    rerank_unifusion_retrieval = policy_mod.rerank_unifusion_retrieval
    UnifusionMoERetrievalConfig = policy_mod.UnifusionMoERetrievalConfig
    rerank_unifusion_moe_retrieval = policy_mod.rerank_unifusion_moe_retrieval
    UnifusionRetryAgentConfig = policy_mod.UnifusionRetryAgentConfig
    select_unifusion_retry_context = policy_mod.select_unifusion_retry_context
    UnifusionTableNumberAgentConfig = policy_mod.UnifusionTableNumberAgentConfig
    select_unifusion_table_number_answer = policy_mod.select_unifusion_table_number_answer
    is_no_evidence_answer = policy_mod.is_no_evidence_answer
except Exception:
    policy_mod = None
    UnifusionPolicyConfig = None
    select_unifusion_policy_context = None
    UnifusionRetrievalAgentConfig = None
    rerank_unifusion_retrieval = None
    UnifusionMoERetrievalConfig = None
    rerank_unifusion_moe_retrieval = None
    UnifusionRetryAgentConfig = None
    select_unifusion_retry_context = None
    UnifusionTableNumberAgentConfig = None
    select_unifusion_table_number_answer = None
    is_no_evidence_answer = None

try:
    ser_mod = importlib.import_module("unifusion_exp.e2e.ser_ranker")
...
try:
    graph_evidence_mod = importlib.import_module("unifusion_exp.e2e.graph_evidence")
...
try:
    v9_mod = importlib.import_module("unifusion_exp.e2e.unifusion_v9")
...
try:
    v10_mod = importlib.import_module("unifusion_exp.e2e.unifusion_v10")
...
try:
    source_head_mod = importlib.import_module("unifusion_exp.e2e.source_head_selector")
...
try:
    source_evidence_mod = importlib.import_module("unifusion_exp.e2e.source_evidence_fusion")
...
try:
    pesv_mod = importlib.import_module("unifusion_exp.e2e.pairwise_slot_verifier")
...
try:
    kgcv_mod = importlib.import_module("unifusion_exp.e2e.kg_consistency_verifier")
...
try:
    source_budgeter_mod = importlib.import_module("unifusion_exp.e2e.source_budgeter")
...
try:
    source_action_mod = importlib.import_module("unifusion_exp.e2e.source_action_policy")
...
eval_mod = importlib.import_module("unifusion_exp.e2e.evaluation")
...
def calibrate_unifusion_answer(
    query: str,
    raw_answer: str,
    contexts: list[str],
...
def apply_unifusion_answer_type_guard(
    query: str,
    raw_answer: str,
    contexts: list[str],
    extractive_numeric_consensus: bool = False,
...
def maybe_apply_unifusion_table_number_agent(
    query: str,
    raw_answer: str,
    contexts: list[str],
    ...
    if select_unifusion_table_number_answer is None or UnifusionTableNumberAgentConfig is None:
        raise RuntimeError("unifusion_policy module is required for --unifusion-table-number-agent")
    ...
    force_trigger = (
        float(args.unifusion_table_number_agent_low_support_threshold) >= 0.0
        and debug["base_support"] < float(args.unifusion_table_number_agent_low_support_threshold)
    )
    cfg = UnifusionTableNumberAgentConfig(
        min_score=float(args.unifusion_table_number_agent_min_score),
    )
    candidate_answer, trace = select_unifusion_table_number_answer(
        query=query,
    ...
    accept = candidate_support >= float(args.unifusion_table_number_agent_min_support)
    if not trace.raw_no_evidence and trace.raw_has_literal:
        accept = accept and candidate_support >= debug["base_support"] + float(
            args.unifusion_table_number_agent_margin
        )
    if force_trigger:
        accept = accept and (
            candidate_support >= debug["base_support"] + float(args.unifusion_table_number_agent_margin)
            or debug["base_support"] < float(args.unifusion_table_number_agent_min_support)
        )
...
def refine_unifusion_answer_with_consensus(
    query: str,
    raw_answer: str,
    contexts: list[str],
    extractive_numeric_consensus: bool = False,
...
def build_rankings_for_query(
    ...
    unifusion_candidate_pool_k: int,
    unifusion_retrieval_multi_agent: bool,
    unifusion_retrieval_agent_pool_k: int,
    unifusion_retrieval_dense_pool_k: int,
    unifusion_retrieval_sparse_pool_k: int,
    unifusion_retrieval_preserve_top: int,
    unifusion_retrieval_base_weight: float,
    unifusion_retrieval_dense_weight: float,
    unifusion_retrieval_sparse_weight: float,
    unifusion_retrieval_target_weight: float,
    unifusion_retrieval_coverage_weight: float,
    unifusion_retrieval_diversity_weight: float,
    unifusion_retrieval_dense_rescue_k: int,
    unifusion_retrieval_dense_rescue_pool_k: int,
    unifusion_retrieval_sibling_seed_k: int,
    unifusion_retrieval_sibling_window: int,
    unifusion_retrieval_sibling_weight: float,
    unifusion_retrieval_moe: bool,
    unifusion_moe_pool_k: int,
    unifusion_moe_prf_seed_k: int,
    unifusion_moe_prf_dense_seed_k: int,
    unifusion_moe_prf_sparse_seed_k: int,
    unifusion_moe_prf_max_terms: int,
    unifusion_moe_sibling_seed_k: int,
    unifusion_moe_sibling_window: int,
    unifusion_moe_sibling_weight: float,
    unifusion_ser_bundle: object | None,
    unifusion_ser_candidate_pool_k: int,
    unifusion_ser_dense_pool_k: int,
    unifusion_ser_sparse_pool_k: int,
    unifusion_ser_preserve_top: int,
    unifusion_ser_blend_weight: float,
    unifusion_ser_diversity_weight: float,
    unifusion_ser_evidence_rescue_k: int,
    unifusion_ser_evidence_rescue_pool_k: int,
    unifusion_ser_evidence_preserve_top: int,
    unifusion_ser_evidence_redundancy_weight: float,
    unifusion_ser_evidence_min_gain: float,
    unifusion_ser_plan_adaptive: bool,
    unifusion_ser_plan_dense_weight: float,
    unifusion_ser_plan_sparse_weight: float,
    unifusion_ser_plan_lexical_weight: float,
    unifusion_ser_plan_slot_weight: float,
    unifusion_ser_evidence_set_selection: bool,
    unifusion_ser_evidence_set_preserve_top: int,
    unifusion_ser_evidence_set_pool_k: int,
    unifusion_ser_evidence_set_cardinality_threshold: float,
    unifusion_ser_evidence_set_learned_weight: float,
    unifusion_ser_evidence_set_base_weight: float,
    unifusion_ser_evidence_set_dense_weight: float,
    unifusion_ser_evidence_set_sparse_weight: float,
    unifusion_ser_evidence_set_probe_weight: float,
    unifusion_ser_evidence_set_slot_weight: float,
    unifusion_ser_evidence_set_anchor_weight: float,
    unifusion_ser_evidence_set_family_weight: float,
    unifusion_ser_evidence_set_redundancy_weight: float,
    unifusion_graph_evidence_expansion: bool,
    unifusion_gee_post_rerank: bool,
    unifusion_gee_candidate_pool_k: int,
    unifusion_gee_dense_pool_k: int,
    unifusion_gee_sparse_pool_k: int,
    unifusion_gee_graph_seed_k: int,
    unifusion_gee_graph_window: int,
    unifusion_gee_preserve_top: int,
    unifusion_gee_trigger_threshold: float,
    unifusion_gee_base_weight: float,
    unifusion_gee_dense_weight: float,
    unifusion_gee_sparse_weight: float,
    unifusion_gee_probe_weight: float,
    unifusion_gee_graph_weight: float,
    unifusion_gee_slot_weight: float,
    unifusion_gee_sibling_weight: float,
    unifusion_gee_redundancy_weight: float,
    doc_id_to_idx: dict[str, int] | None,
    debug_trace: dict[str, list[float]] | None = None,
    unifusion_v9_enabled: bool = False,
    unifusion_v9_local_rerank: bool = False,
    unifusion_v9_dense_pool_k: int = 1200,
    unifusion_v9_sparse_pool_k: int = 1800,
    unifusion_v9_candidate_pool_k: int = 900,
    unifusion_v9_graph_seed_k: int = 36,
    unifusion_v9_graph_window: int = 1,
    unifusion_v9_preserve_top: int = 0,
    unifusion_v9_base_weight: float = 0.28,
    unifusion_v9_dense_weight: float = 0.30,
    unifusion_v9_sparse_weight: float = 0.20,
    unifusion_v9_probe_weight: float = 0.16,
    unifusion_v9_graph_weight: float = 0.08,
    unifusion_v9_slot_weight: float = 0.08,
    unifusion_v9_diversity_weight: float = 0.018,
    unifusion_v9_modality_weight: float = 0.04,
    unifusion_v10_conservative_rerank: bool = False,
    unifusion_v10_preserve_top: int = 1,
    unifusion_v10_direct_preserve_top: int = 2,
    unifusion_v10_reference_pool_k: int = 40,
    unifusion_v10_candidate_pool_k: int = 120,
    unifusion_v10_reference_weight: float = 0.54,
    unifusion_v10_current_weight: float = 0.24,
    unifusion_v10_base_weight: float = 0.10,
    unifusion_v10_dense_weight: float = 0.07,
    unifusion_v10_sparse_weight: float = 0.04,
    unifusion_v10_probe_weight: float = 0.04,
    unifusion_v10_slot_weight: float = 0.03,
    unifusion_v10_diversity_weight: float = 0.012,
    unifusion_v10_margin: float = 0.035,
    unifusion_v10_relevance_floor: float = 0.18,
    unifusion_source_evidence_fusion: bool = False,
    unifusion_source_evidence_slot_verifier_bundle: object | None = None,
    unifusion_source_evidence_topk: int = 5,
    unifusion_source_evidence_candidate_pool_k: int = 80,
    unifusion_source_evidence_preserve_top: int = 1,
    unifusion_source_evidence_base_weight: float = 0.34,
    unifusion_source_evidence_dense_weight: float = 0.16,
    unifusion_source_evidence_sparse_weight: float = 0.08,
    unifusion_source_evidence_reference_weight: float = 0.14,
    unifusion_source_evidence_lexical_weight: float = 0.10,
    unifusion_source_evidence_modality_prior_weight: float = 0.12,
    unifusion_source_evidence_source_balance_weight: float = 0.10,
    unifusion_source_evidence_target_family_weight: float = 0.08,
    unifusion_source_evidence_diversity_weight: float = 0.025,
    unifusion_source_evidence_replacement_margin: float = 0.01,
    unifusion_source_evidence_min_candidate_score: float = 0.08,
    unifusion_source_evidence_dense_guard: bool = False,
    unifusion_source_evidence_dense_guard_topn: int = 5,
    unifusion_source_evidence_dense_guard_prefixes: str = "",
    unifusion_source_evidence_dense_guard_weight: float = 0.22,
    unifusion_source_evidence_dense_rank_weight: float = 0.10,
    unifusion_source_evidence_current_rank_weight: float = 0.06,
    unifusion_source_evidence_source_balance_prefixes: str = "",
    unifusion_source_evidence_max_changed_slots: int = 0,
    unifusion_source_evidence_slot_acceptance_guard: bool = False,
    unifusion_source_evidence_slot_acceptance_prefixes: str = "",
    unifusion_source_evidence_slot_acceptance_margin: float = 0.02,
    unifusion_source_evidence_budget_composer: bool = False,
    unifusion_source_evidence_budget_prefixes: str = "",
    unifusion_source_evidence_budget_candidate_pool_k: int = 180,
    unifusion_source_evidence_budget_start_slot: int = 4,
    unifusion_source_evidence_budget_max_selected: int = 2,
    unifusion_source_evidence_budget_score_weight: float = 0.10,
    unifusion_source_evidence_budget_sibling_weight: float = 0.16,
    unifusion_source_evidence_budget_source_quota_weight: float = 0.08,
    unifusion_source_evidence_budget_tail_rank_weight: float = 0.08,
    unifusion_source_evidence_budget_reference_weight: float = 0.10,
    unifusion_source_evidence_budget_margin: float = 0.006,
    unifusion_source_evidence_budget_redundancy_weight: float = 0.01,
    unifusion_source_evidence_sibling_filler: bool = False,
    unifusion_source_evidence_sibling_filler_prefixes: str = "",
    unifusion_source_evidence_sibling_filler_candidate_pool_k: int = 120,
    unifusion_source_evidence_sibling_filler_start_slot: int = 4,
    unifusion_source_evidence_sibling_filler_max_selected: int = 1,
    unifusion_source_evidence_sibling_filler_tail_topn: int = 10,
    unifusion_source_evidence_sibling_filler_reference_topn: int = 10,
    unifusion_source_evidence_sibling_filler_margin: float = 0.02,
    unifusion_source_evidence_sibling_filler_sibling_weight: float = 0.22,
    unifusion_source_evidence_sibling_filler_reference_weight: float = 0.18,
    unifusion_source_evidence_sibling_filler_tail_weight: float = 0.10,
    unifusion_source_evidence_sibling_filler_dense_weight: float = 0.08,
    unifusion_source_evidence_sibling_filler_source_weight: float = 0.08,
    unifusion_source_evidence_sibling_filler_redundancy_weight: float = 0.008,
    unifusion_source_evidence_slot_verifier: bool = False,
    unifusion_source_evidence_slot_verifier_prefixes: str = "",
    unifusion_source_evidence_slot_verifier_candidate_pool_k: int = 220,
    unifusion_source_evidence_slot_verifier_start_slot: int = 4,
    unifusion_source_evidence_slot_verifier_max_selected: int = 2,
    unifusion_source_evidence_slot_verifier_tail_topn: int = 12,
    unifusion_source_evidence_slot_verifier_reference_topn: int = 12,
    unifusion_source_evidence_slot_verifier_dense_topn: int = 24,
    unifusion_source_evidence_slot_verifier_margin: float = 0.025,
    unifusion_source_evidence_slot_verifier_min_score: float = 0.42,
    unifusion_source_evidence_slot_verifier_model_threshold: float = 0.68,
    unifusion_source_evidence_slot_verifier_static_weight: float = 0.20,
    unifusion_source_evidence_slot_verifier_reference_weight: float = 0.20,
    unifusion_source_evidence_slot_verifier_dense_weight: float = 0.14,
    unifusion_source_evidence_slot_verifier_tail_weight: float = 0.12,
    unifusion_source_evidence_slot_verifier_sibling_weight: float = 0.12,
    unifusion_source_evidence_slot_verifier_source_weight: float = 0.10,
    unifusion_source_evidence_slot_verifier_lexical_weight: float = 0.08,
    unifusion_source_evidence_slot_verifier_family_weight: float = 0.06,
    unifusion_source_evidence_slot_verifier_redundancy_weight: float = 0.012,
    unifusion_source_evidence_kg_preservation_guard: bool = False,
    unifusion_source_evidence_kg_preservation_prefixes: str = "cwq,webqsp",
    unifusion_source_evidence_kg_preservation_min_kg: int = 1,
    unifusion_source_evidence_kg_preservation_candidate_pool_k: int = 160,
    unifusion_source_evidence_kg_preservation_start_slot: int = 2,
    unifusion_source_evidence_kg_preservation_margin: float = 0.015,
    unifusion_source_evidence_kg_preservation_reference_weight: float = 0.24,
    unifusion_source_evidence_kg_preservation_dense_weight: float = 0.16,
    unifusion_source_evidence_kg_preservation_current_weight: float = 0.12,
    unifusion_source_evidence_kg_preservation_family_weight: float = 0.10,
    unifusion_source_evidence_kg_preservation_lexical_weight: float = 0.06,
    unifusion_source_evidence_kg_verifier_bundle: object | None = None,
    unifusion_source_evidence_kg_verifier_weight: float = 0.0,
    unifusion_source_evidence_kg_verifier_min_score: float = 0.0,
    unifusion_source_evidence_kg_verify_existing: bool = False,
    unifusion_source_evidence_kg_verify_existing_max_replacements: int = 1,
    unifusion_source_budgeter_bundle: object | None = None,
    unifusion_source_budgeter_top1_guard: bool = False,
    unifusion_source_budgeter_need_threshold: float = 0.45,
    unifusion_source_budgeter_non_kg_top1_max_kg: int = 1,
    unifusion_source_head_selector: bool = False,
    unifusion_source_head_topn: int = 5,
    unifusion_source_head_source_weight: float = 0.42,
    unifusion_source_head_same_query_weight: float = 0.16,
    unifusion_source_head_position_weight: float = 0.16,
    unifusion_source_head_reference_weight: float = 0.12,
    unifusion_source_head_lexical_weight: float = 0.10,
    unifusion_source_head_base_weight: float = 0.08,
    unifusion_source_head_dense_weight: float = 0.05,
    unifusion_source_head_sparse_weight: float = 0.04,
    unifusion_source_head_margin: float = 0.015,
    unifusion_source_head_off_source_margin: float = 0.04,
    unifusion_source_action_policy_bundle: object | None = None,
    unifusion_source_action_policy_min_prob: float = 0.42,
    unifusion_source_action_policy_topk: int = 5,
    unifusion_source_action_policy_pool_k: int = 10,
    unifusion_final_evidence_composer: bool = False,
    unifusion_final_evidence_topk: int = 5,
    unifusion_final_evidence_candidate_pool_k: int = 120,
    unifusion_final_evidence_dense_pool_k: int = 80,
    unifusion_final_evidence_sparse_pool_k: int = 80,
    unifusion_final_evidence_preserve_top: int = 1,
    unifusion_final_evidence_max_replacements: int = 1,
    unifusion_final_evidence_min_candidate_score: float = 0.62,
    unifusion_final_evidence_replacement_margin: float = 0.08,
    unifusion_final_evidence_min_query_overlap: float = 0.0,
    unifusion_final_evidence_source_need_weight: float = 0.035,
    unifusion_final_evidence_redundancy_weight: float = 0.025,
    unifusion_final_evidence_verifier_bundle: object | None = None,
    unifusion_final_evidence_verifier_threshold: float = 0.70,
    unifusion_final_evidence_verifier_margin: float = 0.0,
) -> dict[str, list[int]]:
    ...
    if unifusion_source_budgeter_bundle is not None:
        pred = unifusion_source_budgeter_bundle.predict(query, query_id)
        ...
        if bool(unifusion_source_budgeter_top1_guard) and str(pred.top1_source) in SOURCE_BUDGET_LABELS:
            source_budget_allowed_top1 = {str(pred.top1_source)}
        if debug_trace is not None:
            debug_trace.setdefault("unifusion_source_budgeter_active", []).append(1.0)
            debug_trace.setdefault("unifusion_source_budgeter_top1_source_index", []).append(
                float(SOURCE_BUDGET_LABELS.index(str(pred.top1_source)) if str(pred.top1_source) in SOURCE_BUDGET_LABELS else -1)
            )
            for label in SOURCE_BUDGET_LABELS:
                debug_trace.setdefault(f"unifusion_source_budgeter_top1_prob_{label}", []).append(
                    float(pred.top1_probs.get(label, 0.0))
                )
                debug_trace.setdefault(f"unifusion_source_budgeter_need_prob_{label}", []).append(
                    float(pred.need_probs.get(label, 0.0))
                )
    elif debug_trace is not None:
        debug_trace.setdefault("unifusion_source_budgeter_active", []).append(0.0)
    ...
    # Always keep a dense reference ranking for UniFusion anchoring/diagnostics.
    ...
    # UniFusion ranking uses route-aware fusion + token-level late bonus +
    ...
    scheme2_unc = 0.0
    late_alpha_eff = float(unifusion_late_alpha)
    ...
    if innovation_scheme2:
        ...
        late_alpha_eff = float(unifusion_late_alpha) * (1.0 + 0.25 * scheme2_unc)
    ...
    if (
        bool(unifusion_v10_conservative_rerank)
        and ("unifusion_rag" in selected_method_set or "unifusion_submod" in selected_method_set)
        and len(uni_candidate) > 0
    ):
        pre_pool_k = int(unifusion_candidate_pool_k)
        if pre_pool_k <= 0:
    ...
    v9_cfg = None
    if bool(unifusion_v9_enabled):
        if expand_v9_candidates is None or V9CandidateConfig is None:
            raise RuntimeError("unifusion_v9 module is required for --unifusion-v9")
        v9_cfg = V9CandidateConfig(
            dense_pool_k=int(unifusion_v9_dense_pool_k),
            sparse_pool_k=int(unifusion_v9_sparse_pool_k),
            candidate_pool_k=int(unifusion_v9_candidate_pool_k),
            graph_seed_k=int(unifusion_v9_graph_seed_k),
            graph_window=int(unifusion_v9_graph_window),
            preserve_top=int(unifusion_v9_preserve_top),
            base_weight=float(unifusion_v9_base_weight),
            dense_weight=float(unifusion_v9_dense_weight),
            sparse_weight=float(unifusion_v9_sparse_weight),
            probe_weight=float(unifusion_v9_probe_weight),
            graph_weight=float(unifusion_v9_graph_weight),
            slot_weight=float(unifusion_v9_slot_weight),
            diversity_weight=float(unifusion_v9_diversity_weight),
            modality_weight=float(unifusion_v9_modality_weight),
        )
        v9_seed_order = [
            int(uni_candidate[int(pos)])
            for pos in np.argsort(np.asarray(uni_base_score, dtype=np.float32))[::-1][
                : max(retrieve_topk, min(len(uni_candidate), int(unifusion_v9_graph_seed_k)))
            ].tolist()
        ]
        ...
        rankings["_unifusion_v9_candidate_pool"] = uni_candidate.tolist()
        rankings["_unifusion_v9_seed_order"] = v9_seed_order
        if debug_trace is not None:
            debug_trace.setdefault("unifusion_v9_enabled", []).append(1.0)
            debug_trace.setdefault("unifusion_v9_input_candidates", []).append(
                float(getattr(v9_expand_trace, "input_candidate_count", 0))
            )
            debug_trace.setdefault("unifusion_v9_output_candidates", []).append(
                float(getattr(v9_expand_trace, "output_candidate_count", 0))
            )
            debug_trace.setdefault("unifusion_v9_dense_added", []).append(
                float(getattr(v9_expand_trace, "dense_added", 0))
            )
            debug_trace.setdefault("unifusion_v9_sparse_added", []).append(
                float(getattr(v9_expand_trace, "sparse_added", 0))
            )
            debug_trace.setdefault("unifusion_v9_graph_added", []).append(
                float(getattr(v9_expand_trace, "graph_added", 0))
            )
            debug_trace.setdefault("unifusion_v9_probe_count", []).append(
                float(getattr(v9_expand_trace, "probe_count", 0))
            )
            debug_trace.setdefault("unifusion_v9_complex_need", []).append(
                float(getattr(v9_expand_trace, "complex_need", 0.0))
            )
            debug_trace.setdefault("unifusion_v9_direct_need", []).append(
                float(getattr(v9_expand_trace, "direct_need", 0.0))
            )
    elif debug_trace is not None:
        debug_trace.setdefault("unifusion_v9_enabled", []).append(0.0)

    uni_rank_all: list[int] = []
    uni_nored_rank_all: list[int] = []
    if "unifusion_rag" in selected_method_set or "unifusion_submod" in selected_method_set:
        pool_k = int(unifusion_candidate_pool_k)
        if pool_k <= 0:
    ...
    if (
        bool(unifusion_graph_evidence_expansion)
        and ("unifusion_rag" in selected_method_set or "unifusion_submod" in selected_method_set)
        and uni_rank_all
    ):
        if expand_graph_evidence_candidates is None or GraphEvidenceConfig is None:
            raise RuntimeError("graph_evidence module is required for --unifusion-graph-evidence-expansion")
        gee_cfg = GraphEvidenceConfig(
            candidate_pool_k=int(unifusion_gee_candidate_pool_k),
            dense_pool_k=int(unifusion_gee_dense_pool_k),
            sparse_pool_k=int(unifusion_gee_sparse_pool_k),
            graph_seed_k=int(unifusion_gee_graph_seed_k),
            graph_window=int(unifusion_gee_graph_window),
            preserve_top=int(unifusion_gee_preserve_top),
            trigger_threshold=float(unifusion_gee_trigger_threshold),
            base_weight=float(unifusion_gee_base_weight),
            dense_weight=float(unifusion_gee_dense_weight),
            sparse_weight=float(unifusion_gee_sparse_weight),
            probe_weight=float(unifusion_gee_probe_weight),
            graph_weight=float(unifusion_gee_graph_weight),
            slot_weight=float(unifusion_gee_slot_weight),
            sibling_weight=float(unifusion_gee_sibling_weight),
            redundancy_weight=float(unifusion_gee_redundancy_weight),
        )
        ...
        if debug_trace is not None:
            debug_trace.setdefault("unifusion_gee_expand_triggered", []).append(
                float(getattr(gee_expand_trace, "triggered", False))
            )
            debug_trace.setdefault("unifusion_gee_expand_input_candidates", []).append(
                float(getattr(gee_expand_trace, "input_candidate_count", 0))
            )
            debug_trace.setdefault("unifusion_gee_expand_output_candidates", []).append(
                float(getattr(gee_expand_trace, "output_candidate_count", 0))
            )
            debug_trace.setdefault("unifusion_gee_expand_graph_added", []).append(
                float(getattr(gee_expand_trace, "graph_added", 0))
            )
            debug_trace.setdefault("unifusion_gee_expand_boosted_existing", []).append(
                float(getattr(gee_expand_trace, "boosted_existing", 0))
            )
            debug_trace.setdefault("unifusion_gee_expand_probe_count", []).append(
                float(getattr(gee_expand_trace, "probe_count", 0))
            )
            debug_trace.setdefault("unifusion_gee_expand_coverage_need", []).append(
                float(getattr(gee_expand_trace, "coverage_need", 0.0))
            )
            debug_trace.setdefault("unifusion_gee_expand_complex_need", []).append(
                float(getattr(gee_expand_trace, "complex_need", 0.0))
            )
            debug_trace.setdefault("unifusion_gee_expand_trigger_score", []).append(
                float(getattr(gee_expand_trace, "trigger_score", 0.0))
            )

    if (
        bool(unifusion_retrieval_multi_agent)
        and ("unifusion_rag" in selected_method_set or "unifusion_submod" in selected_method_set)
        and uni_rank_all
    ):
        if rerank_unifusion_retrieval is None or UnifusionRetrievalAgentConfig is None:
            raise RuntimeError("unifusion_policy module is required for --unifusion-retrieval-multi-agent")
        retrieval_agent_cfg = UnifusionRetrievalAgentConfig(
            candidate_pool_k=int(unifusion_retrieval_agent_pool_k),
            dense_pool_k=int(unifusion_retrieval_dense_pool_k),
            sparse_pool_k=int(unifusion_retrieval_sparse_pool_k),
            preserve_top=int(unifusion_retrieval_preserve_top),
            base_weight=float(unifusion_retrieval_base_weight),
            dense_weight=float(unifusion_retrieval_dense_weight),
            sparse_weight=float(unifusion_retrieval_sparse_weight),
            target_weight=float(unifusion_retrieval_target_weight),
            coverage_weight=float(unifusion_retrieval_coverage_weight),
            diversity_weight=float(unifusion_retrieval_diversity_weight),
            dense_rescue_k=int(unifusion_retrieval_dense_rescue_k),
            dense_rescue_pool_k=int(unifusion_retrieval_dense_rescue_pool_k),
            sibling_seed_k=int(unifusion_retrieval_sibling_seed_k),
            sibling_window=int(unifusion_retrieval_sibling_window),
            sibling_weight=float(unifusion_retrieval_sibling_weight),
        )
        uni_rank_all, retrieval_agent_trace = rerank_unifusion_retrieval(
            query=query,
            current_ranked_idxs=uni_rank_all,
            candidate_idxs=uni_candidate,
        ...
        if debug_trace is not None:
            debug_trace.setdefault("unifusion_retrieval_agent_preserve_count", []).append(
                float(retrieval_agent_trace.preserve_count)
            )
            debug_trace.setdefault("unifusion_retrieval_agent_dense_added", []).append(
                float(retrieval_agent_trace.dense_added)
            )
            debug_trace.setdefault("unifusion_retrieval_agent_sparse_added", []).append(
                float(retrieval_agent_trace.sparse_added)
            )
            debug_trace.setdefault("unifusion_retrieval_agent_dense_rescue_added", []).append(
                float(retrieval_agent_trace.dense_rescue_added)
            )
            debug_trace.setdefault("unifusion_retrieval_agent_sibling_added", []).append(
                float(retrieval_agent_trace.sibling_added)
            )
            debug_trace.setdefault("unifusion_retrieval_agent_forced_hits", []).append(
                float(retrieval_agent_trace.forced_hits)
            )
            debug_trace.setdefault("unifusion_retrieval_agent_coverage", []).append(
                float(retrieval_agent_trace.coverage)
            )

    if (
        bool(unifusion_retrieval_moe)
        and ("unifusion_rag" in selected_method_set or "unifusion_submod" in selected_method_set)
        and uni_rank_all
    ):
        if rerank_unifusion_moe_retrieval is None or UnifusionMoERetrievalConfig is None:
            raise RuntimeError("unifusion_policy module is required for --unifusion-retrieval-moe")
        moe_cfg = UnifusionMoERetrievalConfig(
            candidate_pool_k=int(unifusion_moe_pool_k),
            prf_seed_k=int(unifusion_moe_prf_seed_k),
            prf_dense_seed_k=int(unifusion_moe_prf_dense_seed_k),
            prf_sparse_seed_k=int(unifusion_moe_prf_sparse_seed_k),
            prf_max_terms=int(unifusion_moe_prf_max_terms),
            sibling_seed_k=int(unifusion_moe_sibling_seed_k),
            sibling_window=int(unifusion_moe_sibling_window),
            sibling_weight=float(unifusion_moe_sibling_weight),
        )
        uni_rank_all, moe_trace = rerank_unifusion_moe_retrieval(
            query=query,
            current_ranked_idxs=uni_rank_all,
            candidate_idxs=uni_candidate,
            candidate_scores=uni_base_score,
        ...
        if debug_trace is not None:
            debug_trace.setdefault("unifusion_moe_table_like", []).append(float(moe_trace.table_like))
            debug_trace.setdefault("unifusion_moe_kg_like", []).append(float(moe_trace.kg_like))
            debug_trace.setdefault("unifusion_moe_prf_terms", []).append(float(moe_trace.prf_terms))
            debug_trace.setdefault("unifusion_moe_sibling_added", []).append(float(moe_trace.sibling_added))
            debug_trace.setdefault("unifusion_moe_coverage", []).append(float(moe_trace.coverage))

    if (
        unifusion_ser_bundle is not None
        and ("unifusion_rag" in selected_method_set or "unifusion_submod" in selected_method_set)
        and uni_rank_all
    ):
        if rerank_with_ser is None or SERRankerConfig is None:
            raise RuntimeError("ser_ranker module is required for --unifusion-ser-ranker")
        ser_cfg = SERRankerConfig(
            preserve_top=int(unifusion_ser_preserve_top),
            candidate_pool_k=int(unifusion_ser_candidate_pool_k),
            dense_pool_k=int(unifusion_ser_dense_pool_k),
            sparse_pool_k=int(unifusion_ser_sparse_pool_k),
            blend_weight=float(unifusion_ser_blend_weight),
            diversity_weight=float(unifusion_ser_diversity_weight),
            evidence_rescue_k=int(unifusion_ser_evidence_rescue_k),
            evidence_rescue_pool_k=int(unifusion_ser_evidence_rescue_pool_k),
            evidence_preserve_top=int(unifusion_ser_evidence_preserve_top),
            evidence_redundancy_weight=float(unifusion_ser_evidence_redundancy_weight),
            evidence_min_gain=float(unifusion_ser_evidence_min_gain),
            plan_adaptive=bool(unifusion_ser_plan_adaptive),
            plan_dense_weight=float(unifusion_ser_plan_dense_weight),
            plan_sparse_weight=float(unifusion_ser_plan_sparse_weight),
            plan_lexical_weight=float(unifusion_ser_plan_lexical_weight),
            plan_slot_weight=float(unifusion_ser_plan_slot_weight),
            evidence_set_selection=bool(unifusion_ser_evidence_set_selection),
            evidence_set_preserve_top=int(unifusion_ser_evidence_set_preserve_top),
            evidence_set_pool_k=int(unifusion_ser_evidence_set_pool_k),
            evidence_set_cardinality_threshold=float(unifusion_ser_evidence_set_cardinality_threshold),
            evidence_set_learned_weight=float(unifusion_ser_evidence_set_learned_weight),
            evidence_set_base_weight=float(unifusion_ser_evidence_set_base_weight),
            evidence_set_dense_weight=float(unifusion_ser_evidence_set_dense_weight),
            evidence_set_sparse_weight=float(unifusion_ser_evidence_set_sparse_weight),
            evidence_set_probe_weight=float(unifusion_ser_evidence_set_probe_weight),
            evidence_set_slot_weight=float(unifusion_ser_evidence_set_slot_weight),
            evidence_set_anchor_weight=float(unifusion_ser_evidence_set_anchor_weight),
            evidence_set_family_weight=float(unifusion_ser_evidence_set_family_weight),
            evidence_set_redundancy_weight=float(unifusion_ser_evidence_set_redundancy_weight),
        )
        ...
        uni_rank_all, ser_trace = rerank_with_ser(
            ...
            source_bucket_fn=source_bucket,
            k=retrieve_topk,
            bundle=unifusion_ser_bundle,
            config=ser_cfg,
            query_text=query,
        )
        if debug_trace is not None:
            debug_trace.setdefault("unifusion_ser_candidate_count", []).append(float(ser_trace.candidate_count))
            debug_trace.setdefault("unifusion_ser_preserve_count", []).append(float(ser_trace.preserve_count))
            debug_trace.setdefault("unifusion_ser_changed_count", []).append(float(ser_trace.changed_count))
            debug_trace.setdefault("unifusion_ser_mean_score", []).append(float(ser_trace.mean_score))
            debug_trace.setdefault("unifusion_ser_max_score", []).append(float(ser_trace.max_score))
            debug_trace.setdefault("unifusion_ser_evidence_rescue_added", []).append(
                float(getattr(ser_trace, "evidence_rescue_added", 0))
            )
            debug_trace.setdefault("unifusion_ser_evidence_rescue_pool", []).append(
                float(getattr(ser_trace, "evidence_rescue_pool", 0))
            )
            debug_trace.setdefault("unifusion_ser_plan_direct_score", []).append(
                float(getattr(ser_trace, "plan_direct_score", 0.0))
            )
            debug_trace.setdefault("unifusion_ser_plan_complex_score", []).append(
                float(getattr(ser_trace, "plan_complex_score", 0.0))
            )
            debug_trace.setdefault("unifusion_ser_plan_slot_count", []).append(
                float(getattr(ser_trace, "plan_slot_count", 0))
            )
            debug_trace.setdefault("unifusion_ser_evidence_set_enabled", []).append(
                float(getattr(ser_trace, "evidence_set_enabled", False))
            )
            debug_trace.setdefault("unifusion_ser_evidence_set_cardinality_need", []).append(
                float(getattr(ser_trace, "evidence_set_cardinality_need", 0.0))
            )
            debug_trace.setdefault("unifusion_ser_evidence_set_slot_coverage", []).append(
                float(getattr(ser_trace, "evidence_set_slot_coverage", 0.0))
            )
            debug_trace.setdefault("unifusion_ser_evidence_set_family_count", []).append(
                float(getattr(ser_trace, "evidence_set_family_count", 0))
            )
            debug_trace.setdefault("unifusion_ser_evidence_set_anchor_hits", []).append(
            ...
            if old_uni_rank:
                ...
                debug_trace.setdefault("unifusion_ser_topk_overlap", []).append(float(overlap))

    v10_pre_local_rank_all = list(uni_rank_all)
    if (
        bool(unifusion_v9_enabled)
        and bool(unifusion_v9_local_rerank)
        and ("unifusion_rag" in selected_method_set or "unifusion_submod" in selected_method_set)
        and uni_rank_all
    ):
        if rerank_v9_local_evidence is None:
            raise RuntimeError("unifusion_v9 module is required for --unifusion-v9-local-rerank")
        ...
        if debug_trace is not None:
            debug_trace.setdefault("unifusion_v9_local_rerank", []).append(1.0)
            debug_trace.setdefault("unifusion_v9_local_changed_count", []).append(
                float(getattr(v9_rerank_trace, "changed_count", 0))
            )
            debug_trace.setdefault("unifusion_v9_local_slot_coverage", []).append(
                float(getattr(v9_rerank_trace, "slot_coverage", 0.0))
            )
            if old_uni_rank:
                overlap = len(set(old_uni_rank[:retrieve_topk]) & set(uni_rank_all[:retrieve_topk])) / max(1, retrieve_topk)
                debug_trace.setdefault("unifusion_v9_local_topk_overlap", []).append(float(overlap))
    elif debug_trace is not None and bool(unifusion_v9_enabled):
        debug_trace.setdefault("unifusion_v9_local_rerank", []).append(0.0)

    if (
        bool(unifusion_v10_conservative_rerank)
        and not bool(unifusion_gee_post_rerank)
        and ("unifusion_rag" in selected_method_set or "unifusion_submod" in selected_method_set)
        and uni_rank_all
    ):
        if apply_v10_conservative_gate is None or V10RerankConfig is None:
            raise RuntimeError("unifusion_v10 module is required for --unifusion-v10-conservative-rerank")
        v10_cfg = V10RerankConfig(
            preserve_top=int(unifusion_v10_preserve_top),
            direct_preserve_top=int(unifusion_v10_direct_preserve_top),
            reference_pool_k=int(unifusion_v10_reference_pool_k),
            candidate_pool_k=int(unifusion_v10_candidate_pool_k),
            reference_weight=float(unifusion_v10_reference_weight),
            current_weight=float(unifusion_v10_current_weight),
            base_weight=float(unifusion_v10_base_weight),
            dense_weight=float(unifusion_v10_dense_weight),
            sparse_weight=float(unifusion_v10_sparse_weight),
            probe_weight=float(unifusion_v10_probe_weight),
            slot_weight=float(unifusion_v10_slot_weight),
            diversity_weight=float(unifusion_v10_diversity_weight),
            margin=float(unifusion_v10_margin),
            relevance_floor=float(unifusion_v10_relevance_floor),
        )
        old_uni_rank = list(uni_rank_all)
        ...
        if debug_trace is not None:
            debug_trace.setdefault("unifusion_v10_conservative_rerank", []).append(1.0)
            debug_trace.setdefault("unifusion_v10_candidate_count", []).append(
                float(getattr(v10_trace, "candidate_count", 0))
            )
            debug_trace.setdefault("unifusion_v10_reference_count", []).append(
                float(getattr(v10_trace, "reference_count", 0))
            )
            debug_trace.setdefault("unifusion_v10_preserve_count", []).append(
                float(getattr(v10_trace, "preserve_count", 0))
            )
            debug_trace.setdefault("unifusion_v10_restored_from_reference", []).append(
                float(getattr(v10_trace, "restored_from_reference", 0))
            )
            debug_trace.setdefault("unifusion_v10_accepted_new", []).append(
                float(getattr(v10_trace, "accepted_new", 0))
            )
            debug_trace.setdefault("unifusion_v10_rejected_new", []).append(
                float(getattr(v10_trace, "rejected_new", 0))
            )
            debug_trace.setdefault("unifusion_v10_changed_count", []).append(
                float(getattr(v10_trace, "changed_count", 0))
            )
            debug_trace.setdefault("unifusion_v10_direct_need", []).append(
                float(getattr(v10_trace, "direct_need", 0.0))
            )
            debug_trace.setdefault("unifusion_v10_complex_need", []).append(
                float(getattr(v10_trace, "complex_need", 0.0))
            )
            debug_trace.setdefault("unifusion_v10_effective_margin", []).append(
                float(getattr(v10_trace, "effective_margin", 0.0))
            )
            debug_trace.setdefault("unifusion_v10_effective_relevance_floor", []).append(
                float(getattr(v10_trace, "effective_relevance_floor", 0.0))
            )
            debug_trace.setdefault("unifusion_v10_slot_coverage", []).append(
                float(getattr(v10_trace, "slot_coverage", 0.0))
            )
            debug_trace.setdefault("unifusion_v10_reference_topk_overlap_before", []).append(
                float(getattr(v10_trace, "reference_topk_overlap_before", 0.0))
            )
            debug_trace.setdefault("unifusion_v10_reference_topk_overlap_after", []).append(
                float(getattr(v10_trace, "reference_topk_overlap_after", 0.0))
            )
            if old_uni_rank:
                overlap = len(set(old_uni_rank[:retrieve_topk]) & set(uni_rank_all[:retrieve_topk])) / max(1, retrieve_topk)
                debug_trace.setdefault("unifusion_v10_topk_overlap", []).append(float(overlap))
    elif debug_trace is not None and not bool(unifusion_gee_post_rerank):
        debug_trace.setdefault("unifusion_v10_conservative_rerank", []).append(0.0)

    v10_pre_final_rank_all = list(uni_rank_all)
    if (
        bool(unifusion_graph_evidence_expansion)
        and bool(unifusion_gee_post_rerank)
        and ("unifusion_rag" in selected_method_set or "unifusion_submod" in selected_method_set)
        and uni_rank_all
    ):
        ...
        if debug_trace is not None:
            debug_trace.setdefault("unifusion_gee_triggered", []).append(float(getattr(gee_trace, "triggered", False)))
            debug_trace.setdefault("unifusion_gee_pool_size", []).append(float(getattr(gee_trace, "pool_size", 0)))
            debug_trace.setdefault("unifusion_gee_graph_added", []).append(float(getattr(gee_trace, "graph_added", 0)))
            debug_trace.setdefault("unifusion_gee_probe_count", []).append(float(getattr(gee_trace, "probe_count", 0)))
            debug_trace.setdefault("unifusion_gee_coverage_need", []).append(float(getattr(gee_trace, "coverage_need", 0.0)))
            debug_trace.setdefault("unifusion_gee_complex_need", []).append(float(getattr(gee_trace, "complex_need", 0.0)))
            debug_trace.setdefault("unifusion_gee_changed_count", []).append(float(getattr(gee_trace, "changed_count", 0)))
            if old_uni_rank:
                overlap = len(set(old_uni_rank[:retrieve_topk]) & set(uni_rank_all[:retrieve_topk])) / max(1, retrieve_topk)
                debug_trace.setdefault("unifusion_gee_topk_overlap", []).append(float(overlap))
    ...
    if (
        ...
        if debug_trace is not None:
            ...
            if old_uni_rank:
                overlap = len(set(old_uni_rank[:retrieve_topk]) & set(uni_rank_all[:retrieve_topk])) / max(1, retrieve_topk)
                debug_trace.setdefault("unifusion_v10_topk_overlap", []).append(float(overlap))
    elif debug_trace is not None and bool(unifusion_gee_post_rerank):
        debug_trace.setdefault("unifusion_v10_conservative_rerank", []).append(0.0)

    if (
        bool(unifusion_source_evidence_fusion)
        and ("unifusion_rag" in selected_method_set or "unifusion_submod" in selected_method_set)
        and uni_rank_all
    ):
        if apply_source_evidence_fusion is None or SourceEvidenceFusionConfig is None:
            raise RuntimeError("source_evidence_fusion module is required for --unifusion-source-evidence-fusion")
        source_evidence_cfg = SourceEvidenceFusionConfig(
            topk=int(unifusion_source_evidence_topk),
            candidate_pool_k=int(unifusion_source_evidence_candidate_pool_k),
            preserve_top=int(unifusion_source_evidence_preserve_top),
            base_weight=float(unifusion_source_evidence_base_weight),
            dense_weight=float(unifusion_source_evidence_dense_weight),
            sparse_weight=float(unifusion_source_evidence_sparse_weight),
            reference_weight=float(unifusion_source_evidence_reference_weight),
            lexical_weight=float(unifusion_source_evidence_lexical_weight),
            modality_prior_weight=float(unifusion_source_evidence_modality_prior_weight),
            source_balance_weight=float(unifusion_source_evidence_source_balance_weight),
            target_family_weight=float(unifusion_source_evidence_target_family_weight),
            diversity_weight=float(unifusion_source_evidence_diversity_weight),
            replacement_margin=float(unifusion_source_evidence_replacement_margin),
            min_candidate_score=float(unifusion_source_evidence_min_candidate_score),
            dense_guard=bool(unifusion_source_evidence_dense_guard),
            dense_guard_topn=int(unifusion_source_evidence_dense_guard_topn),
            dense_guard_prefixes=str(unifusion_source_evidence_dense_guard_prefixes),
            dense_guard_weight=float(unifusion_source_evidence_dense_guard_weight),
            dense_rank_weight=float(unifusion_source_evidence_dense_rank_weight),
            current_rank_weight=float(unifusion_source_evidence_current_rank_weight),
            source_balance_prefixes=str(unifusion_source_evidence_source_balance_prefixes),
            max_changed_slots=int(unifusion_source_evidence_max_changed_slots),
            slot_acceptance_guard=bool(unifusion_source_evidence_slot_acceptance_guard),
            slot_acceptance_prefixes=str(unifusion_source_evidence_slot_acceptance_prefixes),
            slot_acceptance_margin=float(unifusion_source_evidence_slot_acceptance_margin),
            budget_composer=bool(unifusion_source_evidence_budget_composer),
            budget_prefixes=str(unifusion_source_evidence_budget_prefixes),
            budget_candidate_pool_k=int(unifusion_source_evidence_budget_candidate_pool_k),
            budget_start_slot=int(unifusion_source_evidence_budget_start_slot),
            budget_max_selected=int(unifusion_source_evidence_budget_max_selected),
            budget_score_weight=float(unifusion_source_evidence_budget_score_weight),
            budget_sibling_weight=float(unifusion_source_evidence_budget_sibling_weight),
            budget_source_quota_weight=float(unifusion_source_evidence_budget_source_quota_weight),
            budget_tail_rank_weight=float(unifusion_source_evidence_budget_tail_rank_weight),
            budget_reference_weight=float(unifusion_source_evidence_budget_reference_weight),
            budget_margin=float(unifusion_source_evidence_budget_margin),
            budget_redundancy_weight=float(unifusion_source_evidence_budget_redundancy_weight),
            sibling_filler=bool(unifusion_source_evidence_sibling_filler),
            sibling_filler_prefixes=str(unifusion_source_evidence_sibling_filler_prefixes),
            sibling_filler_candidate_pool_k=int(unifusion_source_evidence_sibling_filler_candidate_pool_k),
            sibling_filler_start_slot=int(unifusion_source_evidence_sibling_filler_start_slot),
            sibling_filler_max_selected=int(unifusion_source_evidence_sibling_filler_max_selected),
            sibling_filler_tail_topn=int(unifusion_source_evidence_sibling_filler_tail_topn),
            sibling_filler_reference_topn=int(unifusion_source_evidence_sibling_filler_reference_topn),
            sibling_filler_margin=float(unifusion_source_evidence_sibling_filler_margin),
            sibling_filler_sibling_weight=float(unifusion_source_evidence_sibling_filler_sibling_weight),
            sibling_filler_reference_weight=float(unifusion_source_evidence_sibling_filler_reference_weight),
            sibling_filler_tail_weight=float(unifusion_source_evidence_sibling_filler_tail_weight),
            sibling_filler_dense_weight=float(unifusion_source_evidence_sibling_filler_dense_weight),
            sibling_filler_source_weight=float(unifusion_source_evidence_sibling_filler_source_weight),
            sibling_filler_redundancy_weight=float(unifusion_source_evidence_sibling_filler_redundancy_weight),
            slot_verifier=bool(unifusion_source_evidence_slot_verifier),
            slot_verifier_prefixes=str(unifusion_source_evidence_slot_verifier_prefixes),
            slot_verifier_candidate_pool_k=int(unifusion_source_evidence_slot_verifier_candidate_pool_k),
            slot_verifier_start_slot=int(unifusion_source_evidence_slot_verifier_start_slot),
            slot_verifier_max_selected=int(unifusion_source_evidence_slot_verifier_max_selected),
            slot_verifier_tail_topn=int(unifusion_source_evidence_slot_verifier_tail_topn),
            slot_verifier_reference_topn=int(unifusion_source_evidence_slot_verifier_reference_topn),
            slot_verifier_dense_topn=int(unifusion_source_evidence_slot_verifier_dense_topn),
            slot_verifier_margin=float(unifusion_source_evidence_slot_verifier_margin),
            slot_verifier_min_score=float(unifusion_source_evidence_slot_verifier_min_score),
            slot_verifier_model_threshold=float(unifusion_source_evidence_slot_verifier_model_threshold),
            slot_verifier_static_weight=float(unifusion_source_evidence_slot_verifier_static_weight),
            slot_verifier_reference_weight=float(unifusion_source_evidence_slot_verifier_reference_weight),
            slot_verifier_dense_weight=float(unifusion_source_evidence_slot_verifier_dense_weight),
            slot_verifier_tail_weight=float(unifusion_source_evidence_slot_verifier_tail_weight),
            slot_verifier_sibling_weight=float(unifusion_source_evidence_slot_verifier_sibling_weight),
            slot_verifier_source_weight=float(unifusion_source_evidence_slot_verifier_source_weight),
            slot_verifier_lexical_weight=float(unifusion_source_evidence_slot_verifier_lexical_weight),
            slot_verifier_family_weight=float(unifusion_source_evidence_slot_verifier_family_weight),
            slot_verifier_redundancy_weight=float(unifusion_source_evidence_slot_verifier_redundancy_weight),
            kg_preservation_guard=bool(unifusion_source_evidence_kg_preservation_guard),
            kg_preservation_prefixes=str(unifusion_source_evidence_kg_preservation_prefixes),
            kg_preservation_min_kg=int(unifusion_source_evidence_kg_preservation_min_kg),
            kg_preservation_candidate_pool_k=int(unifusion_source_evidence_kg_preservation_candidate_pool_k),
            kg_preservation_start_slot=int(unifusion_source_evidence_kg_preservation_start_slot),
            kg_preservation_margin=float(unifusion_source_evidence_kg_preservation_margin),
            kg_preservation_reference_weight=float(unifusion_source_evidence_kg_preservation_reference_weight),
            kg_preservation_dense_weight=float(unifusion_source_evidence_kg_preservation_dense_weight),
            kg_preservation_current_weight=float(unifusion_source_evidence_kg_preservation_current_weight),
            kg_preservation_family_weight=float(unifusion_source_evidence_kg_preservation_family_weight),
            kg_preservation_lexical_weight=float(unifusion_source_evidence_kg_preservation_lexical_weight),
            kg_preservation_verifier_weight=float(unifusion_source_evidence_kg_verifier_weight),
            kg_preservation_verifier_min_score=float(unifusion_source_evidence_kg_verifier_min_score),
            kg_preservation_verify_existing=bool(unifusion_source_evidence_kg_verify_existing),
            kg_preservation_verify_existing_max_replacements=int(unifusion_source_evidence_kg_verify_existing_max_replacements),
            source_budget_gate=bool(unifusion_source_budgeter_bundle is not None),
            source_budget_need_threshold=float(unifusion_source_budgeter_need_threshold),
            source_budget_non_kg_top1_max_kg=int(unifusion_source_budgeter_non_kg_top1_max_kg),
        )
        old_uni_rank = list(uni_rank_all)
        reference_groups = [g for g in [v10_pre_final_rank_all, v10_pre_local_rank_all, v10_pre_v9_rank_all] if g]
        uni_rank_all, source_evidence_trace = apply_source_evidence_fusion(
            ...
            slot_verifier_bundle=unifusion_source_evidence_slot_verifier_bundle,
            kg_verifier_bundle=unifusion_source_evidence_kg_verifier_bundle,
            source_budget=source_budget,
        )
        if debug_trace is not None:
            debug_trace.setdefault("unifusion_source_evidence_fusion", []).append(1.0)
            debug_trace.setdefault("unifusion_source_evidence_changed_count", []).append(
                float(getattr(source_evidence_trace, "changed_count", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_candidate_count", []).append(
                float(getattr(source_evidence_trace, "candidate_count", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_preserved_count", []).append(
                float(getattr(source_evidence_trace, "preserved_count", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_rescued_from_below_topk", []).append(
                float(getattr(source_evidence_trace, "rescued_from_below_topk", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_source_count_before", []).append(
                float(getattr(source_evidence_trace, "source_count_before", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_source_count_after", []).append(
                float(getattr(source_evidence_trace, "source_count_after", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_text_count_before", []).append(
                float(getattr(source_evidence_trace, "text_count_before", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_table_count_before", []).append(
                float(getattr(source_evidence_trace, "table_count_before", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_kg_count_before", []).append(
                float(getattr(source_evidence_trace, "kg_count_before", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_text_count_after", []).append(
                float(getattr(source_evidence_trace, "text_count_after", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_table_count_after", []).append(
                float(getattr(source_evidence_trace, "table_count_after", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_kg_count_after", []).append(
                float(getattr(source_evidence_trace, "kg_count_after", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_topk_overlap", []).append(
                float(getattr(source_evidence_trace, "topk_overlap", 0.0))
            )
            debug_trace.setdefault("unifusion_source_evidence_mean_selected_score", []).append(
                float(getattr(source_evidence_trace, "mean_selected_score", 0.0))
            )
            debug_trace.setdefault("unifusion_source_evidence_dense_guard_active", []).append(
                float(getattr(source_evidence_trace, "dense_guard_active", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_source_balance_active", []).append(
                float(getattr(source_evidence_trace, "source_balance_active", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_dense_guard_candidates", []).append(
                float(getattr(source_evidence_trace, "dense_guard_candidates", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_dense_guard_selected", []).append(
                float(getattr(source_evidence_trace, "dense_guard_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_max_changed_slots", []).append(
                float(getattr(source_evidence_trace, "max_changed_slots", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_slot_acceptance_active", []).append(
                float(getattr(source_evidence_trace, "slot_acceptance_active", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_slot_acceptance_rejected", []).append(
                float(getattr(source_evidence_trace, "slot_acceptance_rejected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_slot_acceptance_selected", []).append(
                float(getattr(source_evidence_trace, "slot_acceptance_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_budget_composer_active", []).append(
                float(getattr(source_evidence_trace, "budget_composer_active", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_budget_candidate_count", []).append(
                float(getattr(source_evidence_trace, "budget_candidate_count", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_budget_changed_count", []).append(
                float(getattr(source_evidence_trace, "budget_changed_count", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_budget_tail_selected", []).append(
                float(getattr(source_evidence_trace, "budget_tail_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_budget_sibling_selected", []).append(
                float(getattr(source_evidence_trace, "budget_sibling_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_budget_source_quota_selected", []).append(
                float(getattr(source_evidence_trace, "budget_source_quota_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_budget_reference_selected", []).append(
                float(getattr(source_evidence_trace, "budget_reference_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_sibling_filler_active", []).append(
                float(getattr(source_evidence_trace, "sibling_filler_active", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_sibling_filler_candidate_count", []).append(
                float(getattr(source_evidence_trace, "sibling_filler_candidate_count", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_sibling_filler_changed_count", []).append(
                float(getattr(source_evidence_trace, "sibling_filler_changed_count", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_sibling_filler_tail_selected", []).append(
                float(getattr(source_evidence_trace, "sibling_filler_tail_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_sibling_filler_sibling_selected", []).append(
                float(getattr(source_evidence_trace, "sibling_filler_sibling_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_sibling_filler_reference_selected", []).append(
                float(getattr(source_evidence_trace, "sibling_filler_reference_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_sibling_filler_dense_selected", []).append(
                float(getattr(source_evidence_trace, "sibling_filler_dense_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_sibling_filler_rejected", []).append(
                float(getattr(source_evidence_trace, "sibling_filler_rejected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_slot_verifier_active", []).append(
                float(getattr(source_evidence_trace, "slot_verifier_active", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_slot_verifier_candidate_count", []).append(
                float(getattr(source_evidence_trace, "slot_verifier_candidate_count", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_slot_verifier_changed_count", []).append(
                float(getattr(source_evidence_trace, "slot_verifier_changed_count", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_slot_verifier_accepted", []).append(
                float(getattr(source_evidence_trace, "slot_verifier_accepted", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_slot_verifier_rejected", []).append(
                float(getattr(source_evidence_trace, "slot_verifier_rejected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_slot_verifier_tail_selected", []).append(
                float(getattr(source_evidence_trace, "slot_verifier_tail_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_slot_verifier_sibling_selected", []).append(
                float(getattr(source_evidence_trace, "slot_verifier_sibling_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_slot_verifier_reference_selected", []).append(
                float(getattr(source_evidence_trace, "slot_verifier_reference_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_slot_verifier_dense_selected", []).append(
                float(getattr(source_evidence_trace, "slot_verifier_dense_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_slot_verifier_source_selected", []).append(
                float(getattr(source_evidence_trace, "slot_verifier_source_selected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_slot_verifier_model_active", []).append(
                float(getattr(source_evidence_trace, "slot_verifier_model_active", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_kg_guard_active", []).append(
                float(getattr(source_evidence_trace, "kg_guard_active", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_kg_guard_candidate_count", []).append(
                float(getattr(source_evidence_trace, "kg_guard_candidate_count", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_kg_guard_recovered", []).append(
                float(getattr(source_evidence_trace, "kg_guard_recovered", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_kg_guard_rejected", []).append(
                float(getattr(source_evidence_trace, "kg_guard_rejected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_kg_guard_verifier_active", []).append(
                float(getattr(source_evidence_trace, "kg_guard_verifier_active", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_kg_guard_verifier_mean_score", []).append(
                float(getattr(source_evidence_trace, "kg_guard_verifier_mean_score", 0.0))
            )
            debug_trace.setdefault("unifusion_source_evidence_kg_guard_verifier_rejected", []).append(
                float(getattr(source_evidence_trace, "kg_guard_verifier_rejected", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_source_budget_gate_active", []).append(
                float(getattr(source_evidence_trace, "source_budget_gate_active", 0))
            )
            debug_trace.setdefault("unifusion_source_evidence_kg_guard_effective_min_kg", []).append(
                float(getattr(source_evidence_trace, "kg_guard_effective_min_kg", 0))
            )
            if old_uni_rank:
                overlap = len(set(old_uni_rank[:retrieve_topk]) & set(uni_rank_all[:retrieve_topk])) / max(1, retrieve_topk)
                debug_trace.setdefault("unifusion_source_evidence_full_topk_overlap", []).append(float(overlap))
    elif debug_trace is not None:
        debug_trace.setdefault("unifusion_source_evidence_fusion", []).append(0.0)

    if (
        bool(unifusion_source_head_selector)
        and ("unifusion_rag" in selected_method_set or "unifusion_submod" in selected_method_set)
        and uni_rank_all
    ):
        if apply_source_aware_head_selector is None or SourceHeadSelectorConfig is None:
            raise RuntimeError("source_head_selector module is required for --unifusion-source-head-selector")
        head_cfg = SourceHeadSelectorConfig(
            topn=int(unifusion_source_head_topn),
            source_weight=float(unifusion_source_head_source_weight),
            same_query_weight=float(unifusion_source_head_same_query_weight),
            position_weight=float(unifusion_source_head_position_weight),
            reference_weight=float(unifusion_source_head_reference_weight),
            lexical_weight=float(unifusion_source_head_lexical_weight),
            base_weight=float(unifusion_source_head_base_weight),
            dense_weight=float(unifusion_source_head_dense_weight),
            sparse_weight=float(unifusion_source_head_sparse_weight),
            margin=float(unifusion_source_head_margin),
            off_source_margin=float(unifusion_source_head_off_source_margin),
        )
        old_uni_rank = list(uni_rank_all)
        reference_groups = [g for g in [v10_pre_final_rank_all, v10_pre_local_rank_all, v10_pre_v9_rank_all] if g]
        ...
        if debug_trace is not None:
            debug_trace.setdefault("unifusion_source_head_selector", []).append(1.0)
            debug_trace.setdefault("unifusion_source_head_attempted", []).append(
                float(getattr(head_trace, "attempted", 0))
            )
            debug_trace.setdefault("unifusion_source_head_changed", []).append(
                float(getattr(head_trace, "changed", 0))
            )
            debug_trace.setdefault("unifusion_source_head_aligned_before", []).append(
                float(getattr(head_trace, "top_family_aligned_before", 0))
            )
            debug_trace.setdefault("unifusion_source_head_aligned_after", []).append(
                float(getattr(head_trace, "top_family_aligned_after", 0))
            )
            debug_trace.setdefault("unifusion_source_head_source_candidate_count", []).append(
                float(getattr(head_trace, "source_candidate_count", 0))
            )
            debug_trace.setdefault("unifusion_source_head_selected_rank", []).append(
                float(getattr(head_trace, "selected_rank", 0))
            )
            debug_trace.setdefault("unifusion_source_head_selected_score", []).append(
                float(getattr(head_trace, "selected_score", 0.0))
            )
            debug_trace.setdefault("unifusion_source_head_old_score", []).append(
                float(getattr(head_trace, "old_score", 0.0))
            )
            if old_uni_rank:
                debug_trace.setdefault("unifusion_source_head_top1_changed", []).append(
                    float(bool(uni_rank_all and old_uni_rank[0] != uni_rank_all[0]))
                )
    elif debug_trace is not None:
        debug_trace.setdefault("unifusion_source_head_selector", []).append(0.0)

    if (
        unifusion_source_action_policy_bundle is not None
        and ("unifusion_rag" in selected_method_set or "unifusion_submod" in selected_method_set)
        and uni_rank_all
    ):
        if apply_source_action_to_ranked_idxs is None:
            raise RuntimeError("source_action_policy module is required for --unifusion-source-action-policy-model")
        old_uni_rank = list(uni_rank_all)
        ranked_doc_ids_for_policy = [str(doc_ids[j]) for j in old_uni_rank if 0 <= int(j) < len(doc_ids)]
        action_pred = unifusion_source_action_policy_bundle.predict(
            query_text=query,
            query_id=query_id,
            ranked_doc_ids=ranked_doc_ids_for_policy,
            trace=debug_trace,
        )
        action_labels = list(getattr(unifusion_source_action_policy_bundle, "action_labels", SOURCE_ACTION_LABELS) or [])
        action = str(getattr(action_pred, "action", "keep_current"))
        confidence = float(getattr(action_pred, "confidence", 0.0))
        should_apply = action != "keep_current" and confidence >= float(unifusion_source_action_policy_min_prob)
        if should_apply:
            uni_rank_all = apply_source_action_to_ranked_idxs(
                action,
                uni_rank_all,
                doc_ids,
                topk=int(unifusion_source_action_policy_topk),
                pool_k=int(unifusion_source_action_policy_pool_k),
            )
        if debug_trace is not None:
            overlap = len(set(old_uni_rank[:retrieve_topk]) & set(uni_rank_all[:retrieve_topk])) / max(1, retrieve_topk)
            debug_trace.setdefault("unifusion_source_action_policy_active", []).append(1.0)
            debug_trace.setdefault("unifusion_source_action_policy_applied", []).append(float(should_apply))
            debug_trace.setdefault("unifusion_source_action_policy_confidence", []).append(confidence)
            debug_trace.setdefault("unifusion_source_action_policy_topk_overlap", []).append(float(overlap))
            debug_trace.setdefault("unifusion_source_action_policy_action_index", []).append(
                float(action_labels.index(action) if action in action_labels else -1)
            )
    elif debug_trace is not None:
        debug_trace.setdefault("unifusion_source_action_policy_active", []).append(0.0)

    if (
        bool(unifusion_final_evidence_composer)
        and ("unifusion_rag" in selected_method_set or "unifusion_submod" in selected_method_set)
        and uni_rank_all
    ):
        if compose_final_with_ser is None or FinalEvidenceComposerConfig is None:
            raise RuntimeError("ser_ranker module is required for --unifusion-final-evidence-composer")
        if unifusion_ser_bundle is None:
            raise RuntimeError("--unifusion-final-evidence-composer requires --unifusion-ser-ranker")
        final_cfg = FinalEvidenceComposerConfig(
            topk=int(unifusion_final_evidence_topk),
            preserve_top=int(unifusion_final_evidence_preserve_top),
            candidate_pool_k=int(unifusion_final_evidence_candidate_pool_k),
            dense_pool_k=int(unifusion_final_evidence_dense_pool_k),
            sparse_pool_k=int(unifusion_final_evidence_sparse_pool_k),
            max_replacements=int(unifusion_final_evidence_max_replacements),
            min_candidate_score=float(unifusion_final_evidence_min_candidate_score),
            replacement_margin=float(unifusion_final_evidence_replacement_margin),
            min_query_overlap=float(unifusion_final_evidence_min_query_overlap),
            source_need_weight=float(unifusion_final_evidence_source_need_weight),
            redundancy_weight=float(unifusion_final_evidence_redundancy_weight),
            replacement_verifier_threshold=float(unifusion_final_evidence_verifier_threshold),
            replacement_verifier_margin=float(unifusion_final_evidence_verifier_margin),
        )
        old_uni_rank = list(uni_rank_all)
        uni_rank_all, final_trace = compose_final_with_ser(
            ...
            source_budget=source_budget,
            doc_texts=doc_texts,
            replacement_verifier_bundle=unifusion_final_evidence_verifier_bundle,
        )
        if debug_trace is not None:
            topk = max(1, int(unifusion_final_evidence_topk))
            overlap = len(set(old_uni_rank[:topk]) & set(uni_rank_all[:topk])) / max(1, topk)
            debug_trace.setdefault("unifusion_final_evidence_composer", []).append(1.0)
            debug_trace.setdefault("unifusion_final_evidence_candidate_count", []).append(
                float(getattr(final_trace, "candidate_count", 0))
            )
            debug_trace.setdefault("unifusion_final_evidence_preserve_count", []).append(
                float(getattr(final_trace, "preserve_count", 0))
            )
            debug_trace.setdefault("unifusion_final_evidence_changed_count", []).append(
                float(getattr(final_trace, "changed_count", 0))
            )
            debug_trace.setdefault("unifusion_final_evidence_replacement_count", []).append(
                float(getattr(final_trace, "replacement_count", 0))
            )
            debug_trace.setdefault("unifusion_final_evidence_rejected_count", []).append(
                float(getattr(final_trace, "rejected_count", 0))
            )
            debug_trace.setdefault("unifusion_final_evidence_mean_candidate_score", []).append(
                float(getattr(final_trace, "mean_candidate_score", 0.0))
            )
            debug_trace.setdefault("unifusion_final_evidence_max_candidate_score", []).append(
                float(getattr(final_trace, "max_candidate_score", 0.0))
            )
            debug_trace.setdefault("unifusion_final_evidence_min_replaced_score", []).append(
                float(getattr(final_trace, "min_replaced_score", 0.0))
            )
            debug_trace.setdefault("unifusion_final_evidence_max_inserted_score", []).append(
                float(getattr(final_trace, "max_inserted_score", 0.0))
            )
            debug_trace.setdefault("unifusion_final_evidence_topk_overlap", []).append(float(overlap))
            debug_trace.setdefault("unifusion_final_evidence_verifier_active", []).append(
                float(getattr(final_trace, "verifier_active", 0))
            )
            debug_trace.setdefault("unifusion_final_evidence_verifier_scored", []).append(
                float(getattr(final_trace, "verifier_scored", 0))
            )
            debug_trace.setdefault("unifusion_final_evidence_verifier_accepted", []).append(
                float(getattr(final_trace, "verifier_accepted", 0))
            )
            debug_trace.setdefault("unifusion_final_evidence_verifier_rejected", []).append(
                float(getattr(final_trace, "verifier_rejected", 0))
            )
            debug_trace.setdefault("unifusion_final_evidence_verifier_max_score", []).append(
                float(getattr(final_trace, "verifier_max_score", 0.0))
            )
    elif debug_trace is not None:
        debug_trace.setdefault("unifusion_final_evidence_composer", []).append(0.0)
    ...
    uni_nopath_rank_all = greedy_diverse_topk(
        ...
        redundancy_lambda=0.03,
        redundancy_mode="union",
        candidate_pool_k=int(unifusion_candidate_pool_k) if int(unifusion_candidate_pool_k) > 0 else max(retrieve_topk * 4, 80),
    )
    ...
    if "unifusion_rag" in selected_method_set:
        rankings["unifusion_rag"] = apply_preserve(uni_rank_all)
    if "unifusion_submod" in selected_method_set:
        rankings["unifusion_submod"] = apply_preserve(uni_rank_all)
    if "ablation_no_redundancy_e2e" in selected_method_set:
    ...
    if debug_trace is not None:
        debug_trace.setdefault("ranking_unifusion_ms", []).append((time.perf_counter() - t_profile2) * 1000.0)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Weight of UPO-Lite concept-modality prior signal in UniFusion retrieval ranking. 0 disables this signal.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Penalty weight for cross-modal numeric conflicts during UniFusion retrieval ranking. 0 disables it.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="Number of top UniFusion candidates used as anchors for retrieval conflict penalty.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--unifusion-candidate-pool-k",
        type=int,
        default=80,
        help="Cap on the UniFusion candidate pool before greedy redundancy scoring; 0 keeps the auto fallback.",
    )
    parser.add_argument(
        "--unifusion-retrieval-multi-agent",
        action="store_true",
        help="Enable modular multi-agent retrieval reranking for unifusion_rag only.",
    )
    parser.add_argument("--unifusion-retrieval-agent-pool-k", type=int, default=120)
    parser.add_argument("--unifusion-retrieval-dense-pool-k", type=int, default=80)
    parser.add_argument("--unifusion-retrieval-sparse-pool-k", type=int, default=80)
    parser.add_argument("--unifusion-retrieval-preserve-top", type=int, default=1)
    parser.add_argument("--unifusion-retrieval-base-weight", type=float, default=0.50)
    parser.add_argument("--unifusion-retrieval-dense-weight", type=float, default=0.16)
    parser.add_argument("--unifusion-retrieval-sparse-weight", type=float, default=0.10)
    parser.add_argument("--unifusion-retrieval-target-weight", type=float, default=0.10)
    parser.add_argument("--unifusion-retrieval-coverage-weight", type=float, default=0.06)
    parser.add_argument("--unifusion-retrieval-diversity-weight", type=float, default=0.04)
    parser.add_argument("--unifusion-retrieval-dense-rescue-k", type=int, default=0)
    parser.add_argument("--unifusion-retrieval-dense-rescue-pool-k", type=int, default=12)
    parser.add_argument("--unifusion-retrieval-sibling-seed-k", type=int, default=0)
    parser.add_argument("--unifusion-retrieval-sibling-window", type=int, default=0)
    parser.add_argument("--unifusion-retrieval-sibling-weight", type=float, default=0.0)
    parser.add_argument(
        "--unifusion-retrieval-moe",
        action="store_true",
        help="Enable query-planned mixture-of-experts retrieval reranking for UniFusion.",
    )
    parser.add_argument("--unifusion-moe-pool-k", type=int, default=260)
    parser.add_argument("--unifusion-moe-prf-seed-k", type=int, default=6)
    parser.add_argument("--unifusion-moe-prf-dense-seed-k", type=int, default=6)
    parser.add_argument("--unifusion-moe-prf-sparse-seed-k", type=int, default=6)
    parser.add_argument("--unifusion-moe-prf-max-terms", type=int, default=48)
    parser.add_argument("--unifusion-moe-sibling-seed-k", type=int, default=6)
    parser.add_argument("--unifusion-moe-sibling-window", type=int, default=1)
    parser.add_argument("--unifusion-moe-sibling-weight", type=float, default=0.03)
    parser.add_argument(
        "--unifusion-ser-ranker",
        type=Path,
        default=None,
        help="Path to a trained UniFusion-SER retrieval-level supervised reranker bundle.",
    )
    parser.add_argument("--unifusion-ser-candidate-pool-k", type=int, default=180)
    parser.add_argument("--unifusion-ser-dense-pool-k", type=int, default=120)
    parser.add_argument("--unifusion-ser-sparse-pool-k", type=int, default=120)
    parser.add_argument("--unifusion-ser-preserve-top", type=int, default=1)
    parser.add_argument("--unifusion-ser-blend-weight", type=float, default=0.65)
    parser.add_argument("--unifusion-ser-diversity-weight", type=float, default=0.02)
    parser.add_argument("--unifusion-ser-evidence-rescue-k", type=int, default=0)
    parser.add_argument("--unifusion-ser-evidence-rescue-pool-k", type=int, default=24)
    parser.add_argument("--unifusion-ser-evidence-preserve-top", type=int, default=3)
    parser.add_argument("--unifusion-ser-evidence-redundancy-weight", type=float, default=0.04)
    parser.add_argument("--unifusion-ser-evidence-min-gain", type=float, default=0.03)
    parser.add_argument("--unifusion-ser-plan-adaptive", action="store_true")
    parser.add_argument("--unifusion-ser-plan-dense-weight", type=float, default=0.12)
    parser.add_argument("--unifusion-ser-plan-sparse-weight", type=float, default=0.03)
    parser.add_argument("--unifusion-ser-plan-lexical-weight", type=float, default=0.03)
    parser.add_argument("--unifusion-ser-plan-slot-weight", type=float, default=0.04)
    parser.add_argument("--unifusion-ser-evidence-set-selection", action="store_true")
    parser.add_argument("--unifusion-ser-evidence-set-preserve-top", type=int, default=2)
    parser.add_argument("--unifusion-ser-evidence-set-pool-k", type=int, default=220)
    parser.add_argument("--unifusion-ser-evidence-set-cardinality-threshold", type=float, default=0.46)
    parser.add_argument("--unifusion-ser-evidence-set-learned-weight", type=float, default=0.22)
    parser.add_argument("--unifusion-ser-evidence-set-base-weight", type=float, default=0.30)
    parser.add_argument("--unifusion-ser-evidence-set-dense-weight", type=float, default=0.18)
    parser.add_argument("--unifusion-ser-evidence-set-sparse-weight", type=float, default=0.10)
    parser.add_argument("--unifusion-ser-evidence-set-probe-weight", type=float, default=0.10)
    parser.add_argument("--unifusion-ser-evidence-set-slot-weight", type=float, default=0.12)
    parser.add_argument("--unifusion-ser-evidence-set-anchor-weight", type=float, default=0.16)
    parser.add_argument("--unifusion-ser-evidence-set-family-weight", type=float, default=0.05)
    parser.add_argument("--unifusion-ser-evidence-set-redundancy-weight", type=float, default=0.018)
    parser.add_argument(
        "--unifusion-graph-evidence-expansion",
        action="store_true",
        help="Enable query-decomposed graph evidence candidate expansion before UniFusion-SER retrieval.",
    )
    parser.add_argument(
        "--unifusion-gee-post-rerank",
        action="store_true",
        help="Also run the legacy GEE post-SER reranker. Off by default because v5 hurt top-5 metrics.",
    )
    parser.add_argument("--unifusion-gee-candidate-pool-k", type=int, default=420)
    parser.add_argument("--unifusion-gee-dense-pool-k", type=int, default=260)
    parser.add_argument("--unifusion-gee-sparse-pool-k", type=int, default=220)
    parser.add_argument("--unifusion-gee-graph-seed-k", type=int, default=18)
    parser.add_argument("--unifusion-gee-graph-window", type=int, default=1)
    parser.add_argument("--unifusion-gee-preserve-top", type=int, default=2)
    parser.add_argument("--unifusion-gee-trigger-threshold", type=float, default=0.58)
    parser.add_argument("--unifusion-gee-base-weight", type=float, default=0.28)
    parser.add_argument("--unifusion-gee-dense-weight", type=float, default=0.34)
    parser.add_argument("--unifusion-gee-sparse-weight", type=float, default=0.14)
    parser.add_argument("--unifusion-gee-probe-weight", type=float, default=0.12)
    parser.add_argument("--unifusion-gee-graph-weight", type=float, default=0.06)
    parser.add_argument("--unifusion-gee-slot-weight", type=float, default=0.06)
    parser.add_argument("--unifusion-gee-sibling-weight", type=float, default=0.04)
    parser.add_argument("--unifusion-gee-redundancy-weight", type=float, default=0.012)
    parser.add_argument("--unifusion-v9", action="store_true")
    parser.add_argument("--unifusion-v9-local-rerank", action="store_true")
    parser.add_argument("--unifusion-v9-dense-pool-k", type=int, default=1200)
    parser.add_argument("--unifusion-v9-sparse-pool-k", type=int, default=1800)
    parser.add_argument("--unifusion-v9-candidate-pool-k", type=int, default=900)
    parser.add_argument("--unifusion-v9-graph-seed-k", type=int, default=36)
    parser.add_argument("--unifusion-v9-graph-window", type=int, default=1)
    parser.add_argument("--unifusion-v9-preserve-top", type=int, default=0)
    parser.add_argument("--unifusion-v9-base-weight", type=float, default=0.28)
    parser.add_argument("--unifusion-v9-dense-weight", type=float, default=0.30)
    parser.add_argument("--unifusion-v9-sparse-weight", type=float, default=0.20)
    parser.add_argument("--unifusion-v9-probe-weight", type=float, default=0.16)
    parser.add_argument("--unifusion-v9-graph-weight", type=float, default=0.08)
    parser.add_argument("--unifusion-v9-slot-weight", type=float, default=0.08)
    parser.add_argument("--unifusion-v9-diversity-weight", type=float, default=0.018)
    parser.add_argument("--unifusion-v9-modality-weight", type=float, default=0.04)
    parser.add_argument("--unifusion-v10-conservative-rerank", action="store_true")
    parser.add_argument("--unifusion-v10-preserve-top", type=int, default=1)
    parser.add_argument("--unifusion-v10-direct-preserve-top", type=int, default=2)
    parser.add_argument("--unifusion-v10-reference-pool-k", type=int, default=40)
    parser.add_argument("--unifusion-v10-candidate-pool-k", type=int, default=120)
    parser.add_argument("--unifusion-v10-reference-weight", type=float, default=0.54)
    parser.add_argument("--unifusion-v10-current-weight", type=float, default=0.24)
    parser.add_argument("--unifusion-v10-base-weight", type=float, default=0.10)
    parser.add_argument("--unifusion-v10-dense-weight", type=float, default=0.07)
    parser.add_argument("--unifusion-v10-sparse-weight", type=float, default=0.04)
    parser.add_argument("--unifusion-v10-probe-weight", type=float, default=0.04)
    parser.add_argument("--unifusion-v10-slot-weight", type=float, default=0.03)
    parser.add_argument("--unifusion-v10-diversity-weight", type=float, default=0.012)
    parser.add_argument("--unifusion-v10-margin", type=float, default=0.035)
    parser.add_argument("--unifusion-v10-relevance-floor", type=float, default=0.18)
    parser.add_argument("--unifusion-source-evidence-fusion", action="store_true")
    parser.add_argument("--unifusion-source-evidence-topk", type=int, default=5)
    parser.add_argument("--unifusion-source-evidence-candidate-pool-k", type=int, default=80)
    parser.add_argument("--unifusion-source-evidence-preserve-top", type=int, default=1)
    parser.add_argument("--unifusion-source-evidence-base-weight", type=float, default=0.34)
    parser.add_argument("--unifusion-source-evidence-dense-weight", type=float, default=0.16)
    parser.add_argument("--unifusion-source-evidence-sparse-weight", type=float, default=0.08)
    parser.add_argument("--unifusion-source-evidence-reference-weight", type=float, default=0.14)
    parser.add_argument("--unifusion-source-evidence-lexical-weight", type=float, default=0.10)
    parser.add_argument("--unifusion-source-evidence-modality-prior-weight", type=float, default=0.12)
    parser.add_argument("--unifusion-source-evidence-source-balance-weight", type=float, default=0.10)
    parser.add_argument("--unifusion-source-evidence-target-family-weight", type=float, default=0.08)
    parser.add_argument("--unifusion-source-evidence-diversity-weight", type=float, default=0.025)
    parser.add_argument("--unifusion-source-evidence-replacement-margin", type=float, default=0.01)
    parser.add_argument("--unifusion-source-evidence-min-candidate-score", type=float, default=0.08)
    parser.add_argument("--unifusion-source-evidence-dense-guard", action="store_true")
    parser.add_argument("--unifusion-source-evidence-dense-guard-topn", type=int, default=5)
    parser.add_argument("--unifusion-source-evidence-dense-guard-prefixes", type=str, default="")
    parser.add_argument("--unifusion-source-evidence-dense-guard-weight", type=float, default=0.22)
    parser.add_argument("--unifusion-source-evidence-dense-rank-weight", type=float, default=0.10)
    parser.add_argument("--unifusion-source-evidence-current-rank-weight", type=float, default=0.06)
    parser.add_argument("--unifusion-source-evidence-source-balance-prefixes", type=str, default="")
    parser.add_argument("--unifusion-source-evidence-max-changed-slots", type=int, default=0)
    parser.add_argument("--unifusion-source-evidence-slot-acceptance-guard", action="store_true")
    parser.add_argument("--unifusion-source-evidence-slot-acceptance-prefixes", type=str, default="")
    parser.add_argument("--unifusion-source-evidence-slot-acceptance-margin", type=float, default=0.02)
    parser.add_argument("--unifusion-source-evidence-budget-composer", action="store_true")
    parser.add_argument("--unifusion-source-evidence-budget-prefixes", type=str, default="")
    parser.add_argument("--unifusion-source-evidence-budget-candidate-pool-k", type=int, default=180)
    parser.add_argument("--unifusion-source-evidence-budget-start-slot", type=int, default=4)
    parser.add_argument("--unifusion-source-evidence-budget-max-selected", type=int, default=2)
    parser.add_argument("--unifusion-source-evidence-budget-score-weight", type=float, default=0.10)
    parser.add_argument("--unifusion-source-evidence-budget-sibling-weight", type=float, default=0.16)
    parser.add_argument("--unifusion-source-evidence-budget-source-quota-weight", type=float, default=0.08)
    parser.add_argument("--unifusion-source-evidence-budget-tail-rank-weight", type=float, default=0.08)
    parser.add_argument("--unifusion-source-evidence-budget-reference-weight", type=float, default=0.10)
    parser.add_argument("--unifusion-source-evidence-budget-margin", type=float, default=0.006)
    parser.add_argument("--unifusion-source-evidence-budget-redundancy-weight", type=float, default=0.01)
    parser.add_argument("--unifusion-source-evidence-sibling-filler", action="store_true")
    parser.add_argument("--unifusion-source-evidence-sibling-filler-prefixes", type=str, default="")
    parser.add_argument("--unifusion-source-evidence-sibling-filler-candidate-pool-k", type=int, default=120)
    parser.add_argument("--unifusion-source-evidence-sibling-filler-start-slot", type=int, default=4)
    parser.add_argument("--unifusion-source-evidence-sibling-filler-max-selected", type=int, default=1)
    parser.add_argument("--unifusion-source-evidence-sibling-filler-tail-topn", type=int, default=10)
    parser.add_argument("--unifusion-source-evidence-sibling-filler-reference-topn", type=int, default=10)
    parser.add_argument("--unifusion-source-evidence-sibling-filler-margin", type=float, default=0.02)
    parser.add_argument("--unifusion-source-evidence-sibling-filler-sibling-weight", type=float, default=0.22)
    parser.add_argument("--unifusion-source-evidence-sibling-filler-reference-weight", type=float, default=0.18)
    parser.add_argument("--unifusion-source-evidence-sibling-filler-tail-weight", type=float, default=0.10)
    parser.add_argument("--unifusion-source-evidence-sibling-filler-dense-weight", type=float, default=0.08)
    parser.add_argument("--unifusion-source-evidence-sibling-filler-source-weight", type=float, default=0.08)
    parser.add_argument("--unifusion-source-evidence-sibling-filler-redundancy-weight", type=float, default=0.008)
    parser.add_argument(
        "--unifusion-source-evidence-slot-verifier",
        action="store_true",
        help="Enable evidence slot verifier for conservative learned-style top-k tail replacements.",
    )
    parser.add_argument("--unifusion-source-evidence-slot-verifier-prefixes", type=str, default="")
    parser.add_argument("--unifusion-source-evidence-slot-verifier-candidate-pool-k", type=int, default=220)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-start-slot", type=int, default=4)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-max-selected", type=int, default=2)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-tail-topn", type=int, default=12)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-reference-topn", type=int, default=12)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-dense-topn", type=int, default=24)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-margin", type=float, default=0.025)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-min-score", type=float, default=0.42)
    parser.add_argument(
        "--unifusion-source-evidence-slot-verifier-model",
        type=Path,
        default=None,
        help="Path to a trained Pairwise Evidence Slot Verifier bundle.",
    )
    parser.add_argument("--unifusion-source-evidence-slot-verifier-model-threshold", type=float, default=0.68)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-static-weight", type=float, default=0.20)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-reference-weight", type=float, default=0.20)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-dense-weight", type=float, default=0.14)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-tail-weight", type=float, default=0.12)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-sibling-weight", type=float, default=0.12)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-source-weight", type=float, default=0.10)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-lexical-weight", type=float, default=0.08)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-family-weight", type=float, default=0.06)
    parser.add_argument("--unifusion-source-evidence-slot-verifier-redundancy-weight", type=float, default=0.012)
    parser.add_argument("--unifusion-source-evidence-kg-preservation-guard", action="store_true")
    parser.add_argument("--unifusion-source-evidence-kg-preservation-prefixes", type=str, default="cwq,webqsp")
    parser.add_argument("--unifusion-source-evidence-kg-preservation-min-kg", type=int, default=1)
    parser.add_argument("--unifusion-source-evidence-kg-preservation-candidate-pool-k", type=int, default=160)
    parser.add_argument("--unifusion-source-evidence-kg-preservation-start-slot", type=int, default=2)
    parser.add_argument("--unifusion-source-evidence-kg-preservation-margin", type=float, default=0.015)
    parser.add_argument("--unifusion-source-evidence-kg-preservation-reference-weight", type=float, default=0.24)
    parser.add_argument("--unifusion-source-evidence-kg-preservation-dense-weight", type=float, default=0.16)
    parser.add_argument("--unifusion-source-evidence-kg-preservation-current-weight", type=float, default=0.12)
    parser.add_argument("--unifusion-source-evidence-kg-preservation-family-weight", type=float, default=0.10)
    parser.add_argument("--unifusion-source-evidence-kg-preservation-lexical-weight", type=float, default=0.06)
    parser.add_argument(
        "--unifusion-source-evidence-kg-verifier-model",
        type=Path,
        default=None,
        help="Optional Entity-Relation Consistency Verifier bundle for KG preservation.",
    )
    parser.add_argument("--unifusion-source-evidence-kg-verifier-weight", type=float, default=0.0)
    parser.add_argument("--unifusion-source-evidence-kg-verifier-min-score", type=float, default=0.0)
    parser.add_argument("--unifusion-source-evidence-kg-verify-existing", action="store_true")
    parser.add_argument("--unifusion-source-evidence-kg-verify-existing-max-replacements", type=int, default=1)
    parser.add_argument(
        "--unifusion-source-budgeter-model",
        type=Path,
        default=None,
        help="Path to the query-adaptive source budgeter bundle trained on train/dev qrel source distributions.",
    )
    parser.add_argument(
        "--unifusion-source-budgeter-top1-guard",
        action="store_true",
        help="Constrain source-head top1 replacement to the budgeter-predicted source.",
    )
    parser.add_argument("--unifusion-source-budgeter-need-threshold", type=float, default=0.45)
    parser.add_argument("--unifusion-source-budgeter-non-kg-top1-max-kg", type=int, default=1)
    parser.add_argument("--unifusion-source-head-selector", action="store_true")
    parser.add_argument("--unifusion-source-head-topn", type=int, default=5)
    parser.add_argument("--unifusion-source-head-source-weight", type=float, default=0.42)
    parser.add_argument("--unifusion-source-head-same-query-weight", type=float, default=0.16)
    parser.add_argument("--unifusion-source-head-position-weight", type=float, default=0.16)
    parser.add_argument("--unifusion-source-head-reference-weight", type=float, default=0.12)
    parser.add_argument("--unifusion-source-head-lexical-weight", type=float, default=0.10)
    parser.add_argument("--unifusion-source-head-base-weight", type=float, default=0.08)
    parser.add_argument("--unifusion-source-head-dense-weight", type=float, default=0.05)
    parser.add_argument("--unifusion-source-head-sparse-weight", type=float, default=0.04)
    parser.add_argument("--unifusion-source-head-margin", type=float, default=0.015)
    parser.add_argument("--unifusion-source-head-off-source-margin", type=float, default=0.04)
    parser.add_argument(
        "--unifusion-source-action-policy-model",
        type=Path,
        default=None,
        help="Path to a trained counterfactual source-action policy bundle.",
    )
    parser.add_argument("--unifusion-source-action-policy-min-prob", type=float, default=0.42)
    parser.add_argument("--unifusion-source-action-policy-topk", type=int, default=5)
    parser.add_argument("--unifusion-source-action-policy-pool-k", type=int, default=10)
    parser.add_argument(
        "--unifusion-final-evidence-composer",
        action="store_true",
        help="Enable the conservative SER-guided final evidence-set composer for top-k retrieval.",
    )
    parser.add_argument("--unifusion-final-evidence-topk", type=int, default=5)
    parser.add_argument("--unifusion-final-evidence-candidate-pool-k", type=int, default=120)
    parser.add_argument("--unifusion-final-evidence-dense-pool-k", type=int, default=80)
    parser.add_argument("--unifusion-final-evidence-sparse-pool-k", type=int, default=80)
    parser.add_argument("--unifusion-final-evidence-preserve-top", type=int, default=1)
    parser.add_argument("--unifusion-final-evidence-max-replacements", type=int, default=1)
    parser.add_argument("--unifusion-final-evidence-min-candidate-score", type=float, default=0.62)
    parser.add_argument("--unifusion-final-evidence-replacement-margin", type=float, default=0.08)
    parser.add_argument("--unifusion-final-evidence-min-query-overlap", type=float, default=0.0)
    parser.add_argument("--unifusion-final-evidence-source-need-weight", type=float, default=0.035)
    parser.add_argument("--unifusion-final-evidence-redundancy-weight", type=float, default=0.025)
    parser.add_argument(
        "--unifusion-final-evidence-verifier-model",
        type=Path,
        default=None,
        help="Optional Evidence Replacement Verifier bundle for final top-k replacement decisions.",
    )
    parser.add_argument("--unifusion-final-evidence-verifier-threshold", type=float, default=0.70)
    parser.add_argument("--unifusion-final-evidence-verifier-margin", type=float, default=0.0)
    parser.add_argument(
        "--unifusion-policy-context",
        action="store_true",
        help="Enable modular UniFusion policy-guided evidence packing for unifusion_rag only.",
    )
    parser.add_argument("--unifusion-policy-pool-k", type=int, default=80)
    parser.add_argument("--unifusion-policy-dense-pool-k", type=int, default=40)
    parser.add_argument("--unifusion-policy-target-weight", type=float, default=0.18)
    parser.add_argument("--unifusion-policy-coverage-weight", type=float, default=0.16)
    parser.add_argument("--unifusion-policy-diversity-weight", type=float, default=0.08)
    parser.add_argument(
        "--unifusion-no-evidence-retry",
        action="store_true",
        help="Retry UniFusion reader calls when the first answer says evidence is missing or support is low.",
    )
    parser.add_argument("--unifusion-no-evidence-retry-context-k", type=int, default=5)
    parser.add_argument("--unifusion-no-evidence-retry-pool-k", type=int, default=120)
    parser.add_argument("--unifusion-no-evidence-retry-dense-pool-k", type=int, default=120)
    parser.add_argument("--unifusion-no-evidence-retry-min-support", type=float, default=0.08)
    parser.add_argument("--unifusion-no-evidence-retry-margin", type=float, default=0.02)
    parser.add_argument("--unifusion-no-evidence-retry-low-support-threshold", type=float, default=-1.0)
    parser.add_argument(
        "--unifusion-table-number-agent",
        action="store_true",
        help="Enable a local table/number evidence agent that can replace weak number/year LLM answers.",
    )
    parser.add_argument("--unifusion-table-number-agent-min-score", type=float, default=0.34)
    parser.add_argument("--unifusion-table-number-agent-min-support", type=float, default=0.08)
    parser.add_argument("--unifusion-table-number-agent-margin", type=float, default=0.02)
    parser.add_argument("--unifusion-table-number-agent-low-support-threshold", type=float, default=0.05)
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Enable evidence-grounded answer calibration for UniFusion-family methods.",
    )
    parser.add_argument(
        "--unifusion-support-retry-threshold",
        type=float,
        default=-1.0,
        help="If >=0, trigger one dense-context retry when UniFusion answer-support score is below this threshold.",
    )
    parser.add_argument(
        "--unifusion-support-retry-margin",
        type=float,
        default=0.08,
        help="Minimum support-score improvement required to accept the retry answer.",
    )
    parser.add_argument(
        "--unifusion-support-retry-targeted-only",
        dest="unifusion_support_retry_targeted_only",
        action="store_true",
        help="Apply support-based retry only for number/year/location queries.",
    )
    parser.add_argument(
        "--unifusion-support-retry-global",
        dest="unifusion_support_retry_targeted_only",
        action="store_false",
        help="Apply support-based retry for all query types.",
    )
    parser.set_defaults(unifusion_support_retry_targeted_only=True)
    parser.add_argument(
        "--unifusion-support-retry-mode",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.12,
        help="Weight of scheme2 cross-modal agreement bonus in UniFusion ranking.",
    )
    parser.add_argument(
        "--scheme2-token-maxsim-weight",
        type=float,
        default=0.02,
        help="Weight of scheme2 token-level MaxSim-like bonus in UniFusion ranking.",
    )
    parser.add_argument(
        "--schemeb-heavy-mode",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.08,
        help="Weight of heavy table cell/row/column encoder score in UniFusion ranking.",
    )
    parser.add_argument(
        "--heavy-kg-path-weight",
        type=float,
        default=0.08,
        help="Weight of heavy KG path encoder score in UniFusion ranking.",
    )
    parser.add_argument(
        "--heavy-token-late-weight",
        type=float,
        default=0.06,
        help="Weight of heavy token-level late interaction score in UniFusion ranking.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Additional top-k candidates to pull from table/kg branch-specific pools for UniFusion candidate expansion.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=1400,
        help="Upper bound of UniFusion candidate pool size after branch expansion.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.04,
        help="Weight of QA-objective reverse constraint score when ranking UniFusion candidates.",
    )
    parser.add_argument(
    ...
    if int(args.unifusion_candidate_pool_k) < 0:
        raise ValueError("unifusion-candidate-pool-k must be >= 0")
    if int(args.unifusion_retrieval_agent_pool_k) < 1:
        raise ValueError("unifusion-retrieval-agent-pool-k must be >= 1")
    if int(args.unifusion_retrieval_dense_pool_k) < 1:
        raise ValueError("unifusion-retrieval-dense-pool-k must be >= 1")
    if int(args.unifusion_retrieval_sparse_pool_k) < 1:
        raise ValueError("unifusion-retrieval-sparse-pool-k must be >= 1")
    if int(args.unifusion_retrieval_preserve_top) < 0:
        raise ValueError("unifusion-retrieval-preserve-top must be >= 0")
    if int(args.unifusion_retrieval_dense_rescue_k) < 0:
        raise ValueError("unifusion-retrieval-dense-rescue-k must be >= 0")
    if int(args.unifusion_retrieval_dense_rescue_pool_k) < 1:
        raise ValueError("unifusion-retrieval-dense-rescue-pool-k must be >= 1")
    if int(args.unifusion_retrieval_sibling_seed_k) < 0:
        raise ValueError("unifusion-retrieval-sibling-seed-k must be >= 0")
    if int(args.unifusion_retrieval_sibling_window) < 0:
        raise ValueError("unifusion-retrieval-sibling-window must be >= 0")
    if float(args.unifusion_retrieval_sibling_weight) < 0.0:
        raise ValueError("unifusion-retrieval-sibling-weight must be >= 0")
    if int(args.unifusion_moe_pool_k) < 1:
        raise ValueError("unifusion-moe-pool-k must be >= 1")
    if int(args.unifusion_moe_prf_seed_k) < 0:
        raise ValueError("unifusion-moe-prf-seed-k must be >= 0")
    if int(args.unifusion_moe_prf_dense_seed_k) < 0:
        raise ValueError("unifusion-moe-prf-dense-seed-k must be >= 0")
    if int(args.unifusion_moe_prf_sparse_seed_k) < 0:
        raise ValueError("unifusion-moe-prf-sparse-seed-k must be >= 0")
    if int(args.unifusion_moe_prf_max_terms) < 1:
        raise ValueError("unifusion-moe-prf-max-terms must be >= 1")
    if int(args.unifusion_moe_sibling_seed_k) < 0:
        raise ValueError("unifusion-moe-sibling-seed-k must be >= 0")
    if int(args.unifusion_moe_sibling_window) < 0:
        raise ValueError("unifusion-moe-sibling-window must be >= 0")
    if float(args.unifusion_moe_sibling_weight) < 0.0:
        raise ValueError("unifusion-moe-sibling-weight must be >= 0")
    if args.unifusion_ser_ranker is not None and not Path(args.unifusion_ser_ranker).exists():
        raise FileNotFoundError(f"unifusion-ser-ranker not found: {args.unifusion_ser_ranker}")
    if int(args.unifusion_ser_candidate_pool_k) < 1:
        raise ValueError("unifusion-ser-candidate-pool-k must be >= 1")
    if int(args.unifusion_ser_dense_pool_k) < 1:
        raise ValueError("unifusion-ser-dense-pool-k must be >= 1")
    if int(args.unifusion_ser_sparse_pool_k) < 1:
        raise ValueError("unifusion-ser-sparse-pool-k must be >= 1")
    if int(args.unifusion_ser_preserve_top) < 0:
        raise ValueError("unifusion-ser-preserve-top must be >= 0")
    if float(args.unifusion_ser_blend_weight) < 0.0 or float(args.unifusion_ser_blend_weight) > 1.0:
        raise ValueError("unifusion-ser-blend-weight must be in [0, 1]")
    if float(args.unifusion_ser_diversity_weight) < 0.0:
        raise ValueError("unifusion-ser-diversity-weight must be >= 0")
    if int(args.unifusion_ser_evidence_rescue_k) < 0:
        raise ValueError("unifusion-ser-evidence-rescue-k must be >= 0")
    if int(args.unifusion_ser_evidence_rescue_pool_k) < 1:
        raise ValueError("unifusion-ser-evidence-rescue-pool-k must be >= 1")
    if int(args.unifusion_ser_evidence_preserve_top) < 0:
        raise ValueError("unifusion-ser-evidence-preserve-top must be >= 0")
    if float(args.unifusion_ser_evidence_redundancy_weight) < 0.0:
        raise ValueError("unifusion-ser-evidence-redundancy-weight must be >= 0")
    if float(args.unifusion_ser_evidence_min_gain) < 0.0:
        raise ValueError("unifusion-ser-evidence-min-gain must be >= 0")
    if int(args.unifusion_ser_evidence_set_preserve_top) < 0:
        raise ValueError("unifusion-ser-evidence-set-preserve-top must be >= 0")
    if int(args.unifusion_ser_evidence_set_pool_k) < 1:
        raise ValueError("unifusion-ser-evidence-set-pool-k must be >= 1")
    if (
        float(args.unifusion_ser_evidence_set_cardinality_threshold) < 0.0
        or float(args.unifusion_ser_evidence_set_cardinality_threshold) > 1.0
    ):
        raise ValueError("unifusion-ser-evidence-set-cardinality-threshold must be in [0, 1]")
    for name in [
        "unifusion_ser_plan_dense_weight",
        "unifusion_ser_plan_sparse_weight",
        "unifusion_ser_plan_lexical_weight",
        "unifusion_ser_plan_slot_weight",
        "unifusion_ser_evidence_set_learned_weight",
        "unifusion_ser_evidence_set_base_weight",
        "unifusion_ser_evidence_set_dense_weight",
        "unifusion_ser_evidence_set_sparse_weight",
        "unifusion_ser_evidence_set_probe_weight",
        "unifusion_ser_evidence_set_slot_weight",
        "unifusion_ser_evidence_set_anchor_weight",
        "unifusion_ser_evidence_set_family_weight",
        "unifusion_ser_evidence_set_redundancy_weight",
        "unifusion_gee_base_weight",
        "unifusion_gee_dense_weight",
        "unifusion_gee_sparse_weight",
        "unifusion_gee_probe_weight",
        "unifusion_gee_graph_weight",
        "unifusion_gee_slot_weight",
        "unifusion_gee_sibling_weight",
        "unifusion_gee_redundancy_weight",
        "unifusion_v10_reference_weight",
        "unifusion_v10_current_weight",
        "unifusion_v10_base_weight",
        "unifusion_v10_dense_weight",
        "unifusion_v10_sparse_weight",
        "unifusion_v10_probe_weight",
        "unifusion_v10_slot_weight",
        "unifusion_v10_diversity_weight",
        "unifusion_source_evidence_base_weight",
        "unifusion_source_evidence_dense_weight",
        "unifusion_source_evidence_sparse_weight",
        "unifusion_source_evidence_reference_weight",
        "unifusion_source_evidence_lexical_weight",
        "unifusion_source_evidence_modality_prior_weight",
        "unifusion_source_evidence_source_balance_weight",
        "unifusion_source_evidence_target_family_weight",
        "unifusion_source_evidence_diversity_weight",
        "unifusion_source_evidence_dense_guard_weight",
        "unifusion_source_evidence_dense_rank_weight",
        "unifusion_source_evidence_current_rank_weight",
        "unifusion_source_evidence_slot_verifier_static_weight",
        "unifusion_source_evidence_slot_verifier_reference_weight",
        "unifusion_source_evidence_slot_verifier_dense_weight",
        "unifusion_source_evidence_slot_verifier_tail_weight",
        "unifusion_source_evidence_slot_verifier_sibling_weight",
        "unifusion_source_evidence_slot_verifier_source_weight",
        "unifusion_source_evidence_slot_verifier_lexical_weight",
        "unifusion_source_evidence_slot_verifier_family_weight",
        "unifusion_source_head_source_weight",
        "unifusion_source_head_same_query_weight",
        "unifusion_source_head_position_weight",
        "unifusion_source_head_reference_weight",
        "unifusion_source_head_lexical_weight",
        "unifusion_source_head_base_weight",
        "unifusion_source_head_dense_weight",
        "unifusion_source_head_sparse_weight",
    ]:
        if float(getattr(args, name)) < 0.0:
            raise ValueError(f"{name.replace('_', '-')} must be >= 0")
    for name in [
        "unifusion_gee_candidate_pool_k",
        "unifusion_gee_dense_pool_k",
        "unifusion_gee_sparse_pool_k",
        "unifusion_gee_graph_seed_k",
    ]:
        if int(getattr(args, name)) < 1:
            raise ValueError(f"{name.replace('_', '-')} must be >= 1")
    if int(args.unifusion_gee_graph_window) < 0:
        raise ValueError("unifusion-gee-graph-window must be >= 0")
    if int(args.unifusion_gee_preserve_top) < 0:
        raise ValueError("unifusion-gee-preserve-top must be >= 0")
    if float(args.unifusion_gee_trigger_threshold) < 0.0 or float(args.unifusion_gee_trigger_threshold) > 1.0:
        raise ValueError("unifusion-gee-trigger-threshold must be in [0, 1]")
    for name in [
        "unifusion_v10_preserve_top",
        "unifusion_v10_direct_preserve_top",
    ]:
        if int(getattr(args, name)) < 0:
            raise ValueError(f"{name.replace('_', '-')} must be >= 0")
    for name in [
        "unifusion_v10_reference_pool_k",
        "unifusion_v10_candidate_pool_k",
    ]:
        if int(getattr(args, name)) < 1:
            raise ValueError(f"{name.replace('_', '-')} must be >= 1")
    if float(args.unifusion_v10_relevance_floor) < 0.0:
        raise ValueError("unifusion-v10-relevance-floor must be >= 0")
    if int(args.unifusion_source_evidence_topk) < 1:
        raise ValueError("unifusion-source-evidence-topk must be >= 1")
    if int(args.unifusion_source_evidence_candidate_pool_k) < 1:
        raise ValueError("unifusion-source-evidence-candidate-pool-k must be >= 1")
    if int(args.unifusion_source_evidence_preserve_top) < 0:
        raise ValueError("unifusion-source-evidence-preserve-top must be >= 0")
    if float(args.unifusion_source_evidence_replacement_margin) < 0.0:
        raise ValueError("unifusion-source-evidence-replacement-margin must be >= 0")
    if float(args.unifusion_source_evidence_min_candidate_score) < 0.0:
        raise ValueError("unifusion-source-evidence-min-candidate-score must be >= 0")
    if int(args.unifusion_source_evidence_dense_guard_topn) < 1:
        raise ValueError("unifusion-source-evidence-dense-guard-topn must be >= 1")
    if int(args.unifusion_source_evidence_max_changed_slots) < 0:
        raise ValueError("unifusion-source-evidence-max-changed-slots must be >= 0")
    if float(args.unifusion_source_evidence_slot_acceptance_margin) < 0.0:
        raise ValueError("unifusion-source-evidence-slot-acceptance-margin must be >= 0")
    if int(args.unifusion_source_evidence_budget_candidate_pool_k) < 1:
        raise ValueError("unifusion-source-evidence-budget-candidate-pool-k must be >= 1")
    if int(args.unifusion_source_evidence_budget_start_slot) < 1:
        raise ValueError("unifusion-source-evidence-budget-start-slot must be >= 1")
    if int(args.unifusion_source_evidence_budget_max_selected) < 0:
        raise ValueError("unifusion-source-evidence-budget-max-selected must be >= 0")
    if float(args.unifusion_source_evidence_budget_margin) < 0.0:
        raise ValueError("unifusion-source-evidence-budget-margin must be >= 0")
    if float(args.unifusion_source_evidence_budget_redundancy_weight) < 0.0:
        raise ValueError("unifusion-source-evidence-budget-redundancy-weight must be >= 0")
    if int(args.unifusion_source_evidence_sibling_filler_candidate_pool_k) < 1:
        raise ValueError("unifusion-source-evidence-sibling-filler-candidate-pool-k must be >= 1")
    if int(args.unifusion_source_evidence_sibling_filler_start_slot) < 1:
        raise ValueError("unifusion-source-evidence-sibling-filler-start-slot must be >= 1")
    if int(args.unifusion_source_evidence_sibling_filler_max_selected) < 0:
        raise ValueError("unifusion-source-evidence-sibling-filler-max-selected must be >= 0")
    if int(args.unifusion_source_evidence_sibling_filler_tail_topn) < 1:
        raise ValueError("unifusion-source-evidence-sibling-filler-tail-topn must be >= 1")
    if int(args.unifusion_source_evidence_sibling_filler_reference_topn) < 1:
        raise ValueError("unifusion-source-evidence-sibling-filler-reference-topn must be >= 1")
    if float(args.unifusion_source_evidence_sibling_filler_margin) < 0.0:
        raise ValueError("unifusion-source-evidence-sibling-filler-margin must be >= 0")
    if float(args.unifusion_source_evidence_sibling_filler_redundancy_weight) < 0.0:
        raise ValueError("unifusion-source-evidence-sibling-filler-redundancy-weight must be >= 0")
    if int(args.unifusion_source_evidence_slot_verifier_candidate_pool_k) < 1:
        raise ValueError("unifusion-source-evidence-slot-verifier-candidate-pool-k must be >= 1")
    if int(args.unifusion_source_evidence_slot_verifier_start_slot) < 1:
        raise ValueError("unifusion-source-evidence-slot-verifier-start-slot must be >= 1")
    if int(args.unifusion_source_evidence_slot_verifier_max_selected) < 0:
        raise ValueError("unifusion-source-evidence-slot-verifier-max-selected must be >= 0")
    if int(args.unifusion_source_evidence_slot_verifier_tail_topn) < 1:
        raise ValueError("unifusion-source-evidence-slot-verifier-tail-topn must be >= 1")
    if int(args.unifusion_source_evidence_slot_verifier_reference_topn) < 1:
        raise ValueError("unifusion-source-evidence-slot-verifier-reference-topn must be >= 1")
    if int(args.unifusion_source_evidence_slot_verifier_dense_topn) < 1:
        raise ValueError("unifusion-source-evidence-slot-verifier-dense-topn must be >= 1")
    if float(args.unifusion_source_evidence_slot_verifier_margin) < 0.0:
        raise ValueError("unifusion-source-evidence-slot-verifier-margin must be >= 0")
    if float(args.unifusion_source_evidence_slot_verifier_min_score) < 0.0:
        raise ValueError("unifusion-source-evidence-slot-verifier-min-score must be >= 0")
    if (
        float(args.unifusion_source_evidence_slot_verifier_model_threshold) < 0.0
        or float(args.unifusion_source_evidence_slot_verifier_model_threshold) > 1.0
    ):
        raise ValueError("unifusion-source-evidence-slot-verifier-model-threshold must be in [0, 1]")
    if args.unifusion_source_evidence_slot_verifier_model is not None and not Path(args.unifusion_source_evidence_slot_verifier_model).exists():
        raise FileNotFoundError(
            f"unifusion-source-evidence-slot-verifier-model not found: {args.unifusion_source_evidence_slot_verifier_model}"
        )
    if float(args.unifusion_source_evidence_slot_verifier_redundancy_weight) < 0.0:
        raise ValueError("unifusion-source-evidence-slot-verifier-redundancy-weight must be >= 0")
    if args.unifusion_source_evidence_kg_verifier_model is not None and not Path(args.unifusion_source_evidence_kg_verifier_model).exists():
        raise FileNotFoundError(
            f"unifusion-source-evidence-kg-verifier-model not found: {args.unifusion_source_evidence_kg_verifier_model}"
        )
    if float(args.unifusion_source_evidence_kg_verifier_weight) < 0.0:
        raise ValueError("unifusion-source-evidence-kg-verifier-weight must be >= 0")
    if (
        float(args.unifusion_source_evidence_kg_verifier_min_score) < 0.0
        or float(args.unifusion_source_evidence_kg_verifier_min_score) > 1.0
    ):
        raise ValueError("unifusion-source-evidence-kg-verifier-min-score must be in [0, 1]")
    if int(args.unifusion_source_evidence_kg_verify_existing_max_replacements) < 0:
        raise ValueError("unifusion-source-evidence-kg-verify-existing-max-replacements must be >= 0")
    if args.unifusion_source_budgeter_model is not None and not Path(args.unifusion_source_budgeter_model).exists():
        raise FileNotFoundError(f"unifusion-source-budgeter-model not found: {args.unifusion_source_budgeter_model}")
    if (
        float(args.unifusion_source_budgeter_need_threshold) < 0.0
        or float(args.unifusion_source_budgeter_need_threshold) > 1.0
    ):
        raise ValueError("unifusion-source-budgeter-need-threshold must be in [0, 1]")
    if int(args.unifusion_source_budgeter_non_kg_top1_max_kg) < 0:
        raise ValueError("unifusion-source-budgeter-non-kg-top1-max-kg must be >= 0")
    if int(args.unifusion_source_head_topn) < 1:
        raise ValueError("unifusion-source-head-topn must be >= 1")
    if float(args.unifusion_source_head_margin) < 0.0:
        raise ValueError("unifusion-source-head-margin must be >= 0")
    if float(args.unifusion_source_head_off_source_margin) < 0.0:
        raise ValueError("unifusion-source-head-off-source-margin must be >= 0")
    if args.unifusion_source_action_policy_model is not None and not Path(
        args.unifusion_source_action_policy_model
    ).exists():
        raise FileNotFoundError(
            f"unifusion-source-action-policy-model not found: {args.unifusion_source_action_policy_model}"
        )
    if (
        float(args.unifusion_source_action_policy_min_prob) < 0.0
        or float(args.unifusion_source_action_policy_min_prob) > 1.0
    ):
        raise ValueError("unifusion-source-action-policy-min-prob must be in [0, 1]")
    if int(args.unifusion_source_action_policy_topk) < 1:
        raise ValueError("unifusion-source-action-policy-topk must be >= 1")
    if int(args.unifusion_source_action_policy_pool_k) < 1:
        raise ValueError("unifusion-source-action-policy-pool-k must be >= 1")
    if int(args.unifusion_final_evidence_topk) < 1:
        raise ValueError("unifusion-final-evidence-topk must be >= 1")
    if int(args.unifusion_final_evidence_candidate_pool_k) < 1:
        raise ValueError("unifusion-final-evidence-candidate-pool-k must be >= 1")
    if int(args.unifusion_final_evidence_dense_pool_k) < 1:
        raise ValueError("unifusion-final-evidence-dense-pool-k must be >= 1")
    if int(args.unifusion_final_evidence_sparse_pool_k) < 1:
        raise ValueError("unifusion-final-evidence-sparse-pool-k must be >= 1")
    if int(args.unifusion_final_evidence_preserve_top) < 0:
        raise ValueError("unifusion-final-evidence-preserve-top must be >= 0")
    if int(args.unifusion_final_evidence_max_replacements) < 0:
        raise ValueError("unifusion-final-evidence-max-replacements must be >= 0")
    for _name in (
        "unifusion_final_evidence_min_candidate_score",
        "unifusion_final_evidence_replacement_margin",
        "unifusion_final_evidence_min_query_overlap",
        "unifusion_final_evidence_source_need_weight",
        "unifusion_final_evidence_redundancy_weight",
        "unifusion_final_evidence_verifier_margin",
    ):
        if float(getattr(args, _name)) < 0.0:
            raise ValueError(f"{_name.replace('_', '-')} must be >= 0")
    if args.unifusion_final_evidence_verifier_model is not None and not Path(
        args.unifusion_final_evidence_verifier_model
    ).exists():
        raise FileNotFoundError(
            f"unifusion-final-evidence-verifier-model not found: {args.unifusion_final_evidence_verifier_model}"
        )
    if (
        float(args.unifusion_final_evidence_verifier_threshold) < 0.0
        or float(args.unifusion_final_evidence_verifier_threshold) > 1.0
    ):
        raise ValueError("unifusion-final-evidence-verifier-threshold must be in [0, 1]")
    for _name in (
        "unifusion_retrieval_base_weight",
        "unifusion_retrieval_dense_weight",
        "unifusion_retrieval_sparse_weight",
        "unifusion_retrieval_target_weight",
        "unifusion_retrieval_coverage_weight",
        "unifusion_retrieval_diversity_weight",
    ):
        if float(getattr(args, _name)) < 0.0:
            raise ValueError(f"{_name.replace('_', '-')} must be >= 0")
    if int(args.unifusion_policy_pool_k) < 1:
        raise ValueError("unifusion-policy-pool-k must be >= 1")
    if int(args.unifusion_policy_dense_pool_k) < 1:
        raise ValueError("unifusion-policy-dense-pool-k must be >= 1")
    if float(args.unifusion_policy_target_weight) < 0.0:
        raise ValueError("unifusion-policy-target-weight must be >= 0")
    if float(args.unifusion_policy_coverage_weight) < 0.0:
        raise ValueError("unifusion-policy-coverage-weight must be >= 0")
    if float(args.unifusion_policy_diversity_weight) < 0.0:
        raise ValueError("unifusion-policy-diversity-weight must be >= 0")
    if int(args.unifusion_no_evidence_retry_context_k) < 1:
        raise ValueError("unifusion-no-evidence-retry-context-k must be >= 1")
    if int(args.unifusion_no_evidence_retry_pool_k) < 1:
        raise ValueError("unifusion-no-evidence-retry-pool-k must be >= 1")
    if int(args.unifusion_no_evidence_retry_dense_pool_k) < 1:
        raise ValueError("unifusion-no-evidence-retry-dense-pool-k must be >= 1")
    if float(args.unifusion_no_evidence_retry_min_support) < -1.0:
        raise ValueError("unifusion-no-evidence-retry-min-support must be >= -1")
    if float(args.unifusion_no_evidence_retry_margin) < 0.0:
        raise ValueError("unifusion-no-evidence-retry-margin must be >= 0")
    if float(args.unifusion_no_evidence_retry_low_support_threshold) < -1.0:
        raise ValueError("unifusion-no-evidence-retry-low-support-threshold must be >= -1")
    if float(args.unifusion_table_number_agent_min_score) < 0.0:
        raise ValueError("unifusion-table-number-agent-min-score must be >= 0")
    if float(args.unifusion_table_number_agent_min_support) < -1.0:
        raise ValueError("unifusion-table-number-agent-min-support must be >= -1")
    if float(args.unifusion_table_number_agent_margin) < 0.0:
        raise ValueError("unifusion-table-number-agent-margin must be >= 0")
    if float(args.unifusion_table_number_agent_low_support_threshold) < -1.0:
        raise ValueError("unifusion-table-number-agent-low-support-threshold must be >= -1")
    ...
    if float(args.unifusion_support_retry_threshold) < -1.0:
        raise ValueError("unifusion-support-retry-threshold must be >= -1")
    if float(args.unifusion_support_retry_margin) < 0.0:
        raise ValueError("unifusion-support-retry-margin must be >= 0")
    if int(args.unifusion_support_retry_pool_k) < 1:
        raise ValueError("unifusion-support-retry-pool-k must be >= 1")
    if float(args.unifusion_support_retry_answer_boost) < 0.0:
        raise ValueError("unifusion-support-retry-answer-boost must be >= 0")
    if int(args.unifusion_support_retry_complexity_min) < 0 or int(args.unifusion_support_retry_complexity_min) > 5:
        raise ValueError("unifusion-support-retry-complexity-min must be in [0,5]")
    if float(args.unifusion_support_retry_entropy_min) < -1.0 or float(args.unifusion_support_retry_entropy_min) > 1.0:
        raise ValueError("unifusion-support-retry-entropy-min must be in [-1,1]")
    if float(args.unifusion_consensus_refine_min_gain) < 0.0:
        raise ValueError("unifusion-consensus-refine-min-gain must be >= 0")
    if float(args.unifusion_consensus_refine_support_threshold) < -1.0:
        raise ValueError("unifusion-consensus-refine-support-threshold must be >= -1")
    if int(args.unifusion_consensus_refine_complexity_min) < 0 or int(args.unifusion_consensus_refine_complexity_min) > 5:
        raise ValueError("unifusion-consensus-refine-complexity-min must be in [0,5]")
    if float(args.unifusion_consensus_refine_entropy_min) < -1.0 or float(args.unifusion_consensus_refine_entropy_min) > 1.0:
        raise ValueError("unifusion-consensus-refine-entropy-min must be in [-1,1]")
    ...
    embed_backend = os.environ.get("UNIFUSION_EMBED_BACKEND", "hf").strip().lower() or "hf"
    query_prefix = os.environ.get("UNIFUSION_QUERY_PREFIX", "")
    doc_prefix = os.environ.get("UNIFUSION_DOC_PREFIX", "")
    print(
    ...
    unifusion_ser_bundle = None
    unifusion_ser_meta_summary = {}
    if args.unifusion_ser_ranker is not None:
        if load_ser_ranker_bundle is None:
            raise RuntimeError("ser_ranker module is required for --unifusion-ser-ranker")
        unifusion_ser_bundle = load_ser_ranker_bundle(args.unifusion_ser_ranker)
        ser_meta = dict(getattr(unifusion_ser_bundle, "meta", {}) or {})
        ...
        print(f"[retrieval] loaded UniFusion-SER ranker: {args.unifusion_ser_ranker}")

    unifusion_slot_verifier_bundle = None
    if args.unifusion_source_evidence_slot_verifier_model is not None:
        if load_pairwise_slot_verifier_bundle is None:
            raise RuntimeError(
                "pairwise_slot_verifier module is required for --unifusion-source-evidence-slot-verifier-model"
            )
        unifusion_slot_verifier_bundle = load_pairwise_slot_verifier_bundle(
            args.unifusion_source_evidence_slot_verifier_model
        )
        print(
            f"[retrieval] loaded Pairwise Evidence Slot Verifier: "
            f"{args.unifusion_source_evidence_slot_verifier_model}"
        )

    unifusion_final_evidence_verifier_bundle = None
    unifusion_final_evidence_verifier_meta_summary = {}
    if args.unifusion_final_evidence_verifier_model is not None:
        if load_pairwise_slot_verifier_bundle is None:
            raise RuntimeError(
                "pairwise_slot_verifier module is required for --unifusion-final-evidence-verifier-model"
            )
        unifusion_final_evidence_verifier_bundle = load_pairwise_slot_verifier_bundle(
            args.unifusion_final_evidence_verifier_model
        )
        erv_meta = dict(getattr(unifusion_final_evidence_verifier_bundle, "metadata", {}) or {})
        unifusion_final_evidence_verifier_meta_summary = {
        ...
        print(
            ...
            f"{args.unifusion_final_evidence_verifier_model}"
        )

    unifusion_kg_verifier_bundle = None
    unifusion_kg_verifier_meta_summary = {}
    if args.unifusion_source_evidence_kg_verifier_model is not None:
        if load_kg_consistency_bundle is None:
            raise RuntimeError(
                "kg_consistency_verifier module is required for --unifusion-source-evidence-kg-verifier-model"
            )
        unifusion_kg_verifier_bundle = load_kg_consistency_bundle(
            args.unifusion_source_evidence_kg_verifier_model
        )
        kg_meta = dict(getattr(unifusion_kg_verifier_bundle, "metadata", {}) or {})
        ...
        print(
            ...
            f"{args.unifusion_source_evidence_kg_verifier_model}"
        )

    unifusion_source_budgeter_bundle = None
    unifusion_source_budgeter_meta_summary = {}
    if args.unifusion_source_budgeter_model is not None:
        if load_source_budgeter_bundle is None:
            raise RuntimeError("source_budgeter module is required for --unifusion-source-budgeter-model")
        unifusion_source_budgeter_bundle = load_source_budgeter_bundle(args.unifusion_source_budgeter_model)
        budget_meta = dict(getattr(unifusion_source_budgeter_bundle, "metadata", {}) or {})
        ...
        print(f"[retrieval] loaded Query-Adaptive Source Budgeter: {args.unifusion_source_budgeter_model}")

    unifusion_source_action_policy_bundle = None
    unifusion_source_action_policy_meta_summary = {}
    if args.unifusion_source_action_policy_model is not None:
        if load_source_action_policy_bundle is None:
            raise RuntimeError("source_action_policy module is required for --unifusion-source-action-policy-model")
        unifusion_source_action_policy_bundle = load_source_action_policy_bundle(args.unifusion_source_action_policy_model)
        action_meta = dict(getattr(unifusion_source_action_policy_bundle, "metadata", {}) or {})
        ...
        print(f"[retrieval] loaded Source Action/Utility Policy: {args.unifusion_source_action_policy_model}")
    ...
    for i, row in enumerate(rows):
        ...
        rankings = build_rankings_for_query(
            ...
            unifusion_late_alpha=args.unifusion_late_alpha,
            ...
            unifusion_candidate_pool_k=int(args.unifusion_candidate_pool_k),
            unifusion_retrieval_multi_agent=bool(args.unifusion_retrieval_multi_agent),
            unifusion_retrieval_agent_pool_k=int(args.unifusion_retrieval_agent_pool_k),
            unifusion_retrieval_dense_pool_k=int(args.unifusion_retrieval_dense_pool_k),
            unifusion_retrieval_sparse_pool_k=int(args.unifusion_retrieval_sparse_pool_k),
            unifusion_retrieval_preserve_top=int(args.unifusion_retrieval_preserve_top),
            unifusion_retrieval_base_weight=float(args.unifusion_retrieval_base_weight),
            unifusion_retrieval_dense_weight=float(args.unifusion_retrieval_dense_weight),
            unifusion_retrieval_sparse_weight=float(args.unifusion_retrieval_sparse_weight),
            unifusion_retrieval_target_weight=float(args.unifusion_retrieval_target_weight),
            unifusion_retrieval_coverage_weight=float(args.unifusion_retrieval_coverage_weight),
            unifusion_retrieval_diversity_weight=float(args.unifusion_retrieval_diversity_weight),
            unifusion_retrieval_dense_rescue_k=int(args.unifusion_retrieval_dense_rescue_k),
            unifusion_retrieval_dense_rescue_pool_k=int(args.unifusion_retrieval_dense_rescue_pool_k),
            unifusion_retrieval_sibling_seed_k=int(args.unifusion_retrieval_sibling_seed_k),
            unifusion_retrieval_sibling_window=int(args.unifusion_retrieval_sibling_window),
            unifusion_retrieval_sibling_weight=float(args.unifusion_retrieval_sibling_weight),
            unifusion_retrieval_moe=bool(args.unifusion_retrieval_moe),
            unifusion_moe_pool_k=int(args.unifusion_moe_pool_k),
            unifusion_moe_prf_seed_k=int(args.unifusion_moe_prf_seed_k),
            unifusion_moe_prf_dense_seed_k=int(args.unifusion_moe_prf_dense_seed_k),
            unifusion_moe_prf_sparse_seed_k=int(args.unifusion_moe_prf_sparse_seed_k),
            unifusion_moe_prf_max_terms=int(args.unifusion_moe_prf_max_terms),
            unifusion_moe_sibling_seed_k=int(args.unifusion_moe_sibling_seed_k),
            unifusion_moe_sibling_window=int(args.unifusion_moe_sibling_window),
            unifusion_moe_sibling_weight=float(args.unifusion_moe_sibling_weight),
            unifusion_ser_bundle=unifusion_ser_bundle,
            unifusion_ser_candidate_pool_k=int(args.unifusion_ser_candidate_pool_k),
            unifusion_ser_dense_pool_k=int(args.unifusion_ser_dense_pool_k),
            unifusion_ser_sparse_pool_k=int(args.unifusion_ser_sparse_pool_k),
            unifusion_ser_preserve_top=int(args.unifusion_ser_preserve_top),
            unifusion_ser_blend_weight=float(args.unifusion_ser_blend_weight),
            unifusion_ser_diversity_weight=float(args.unifusion_ser_diversity_weight),
            unifusion_ser_evidence_rescue_k=int(args.unifusion_ser_evidence_rescue_k),
            unifusion_ser_evidence_rescue_pool_k=int(args.unifusion_ser_evidence_rescue_pool_k),
            unifusion_ser_evidence_preserve_top=int(args.unifusion_ser_evidence_preserve_top),
            unifusion_ser_evidence_redundancy_weight=float(args.unifusion_ser_evidence_redundancy_weight),
            unifusion_ser_evidence_min_gain=float(args.unifusion_ser_evidence_min_gain),
            unifusion_ser_plan_adaptive=bool(args.unifusion_ser_plan_adaptive),
            unifusion_ser_plan_dense_weight=float(args.unifusion_ser_plan_dense_weight),
            unifusion_ser_plan_sparse_weight=float(args.unifusion_ser_plan_sparse_weight),
            unifusion_ser_plan_lexical_weight=float(args.unifusion_ser_plan_lexical_weight),
            unifusion_ser_plan_slot_weight=float(args.unifusion_ser_plan_slot_weight),
            unifusion_ser_evidence_set_selection=bool(args.unifusion_ser_evidence_set_selection),
            unifusion_ser_evidence_set_preserve_top=int(args.unifusion_ser_evidence_set_preserve_top),
            unifusion_ser_evidence_set_pool_k=int(args.unifusion_ser_evidence_set_pool_k),
            unifusion_ser_evidence_set_cardinality_threshold=float(args.unifusion_ser_evidence_set_cardinality_threshold),
            unifusion_ser_evidence_set_learned_weight=float(args.unifusion_ser_evidence_set_learned_weight),
            unifusion_ser_evidence_set_base_weight=float(args.unifusion_ser_evidence_set_base_weight),
            unifusion_ser_evidence_set_dense_weight=float(args.unifusion_ser_evidence_set_dense_weight),
            unifusion_ser_evidence_set_sparse_weight=float(args.unifusion_ser_evidence_set_sparse_weight),
            unifusion_ser_evidence_set_probe_weight=float(args.unifusion_ser_evidence_set_probe_weight),
            unifusion_ser_evidence_set_slot_weight=float(args.unifusion_ser_evidence_set_slot_weight),
            unifusion_ser_evidence_set_anchor_weight=float(args.unifusion_ser_evidence_set_anchor_weight),
            unifusion_ser_evidence_set_family_weight=float(args.unifusion_ser_evidence_set_family_weight),
            unifusion_ser_evidence_set_redundancy_weight=float(args.unifusion_ser_evidence_set_redundancy_weight),
            unifusion_graph_evidence_expansion=bool(args.unifusion_graph_evidence_expansion),
            unifusion_gee_post_rerank=bool(args.unifusion_gee_post_rerank),
            unifusion_gee_candidate_pool_k=int(args.unifusion_gee_candidate_pool_k),
            unifusion_gee_dense_pool_k=int(args.unifusion_gee_dense_pool_k),
            unifusion_gee_sparse_pool_k=int(args.unifusion_gee_sparse_pool_k),
            unifusion_gee_graph_seed_k=int(args.unifusion_gee_graph_seed_k),
            unifusion_gee_graph_window=int(args.unifusion_gee_graph_window),
            unifusion_gee_preserve_top=int(args.unifusion_gee_preserve_top),
            unifusion_gee_trigger_threshold=float(args.unifusion_gee_trigger_threshold),
            unifusion_gee_base_weight=float(args.unifusion_gee_base_weight),
            unifusion_gee_dense_weight=float(args.unifusion_gee_dense_weight),
            unifusion_gee_sparse_weight=float(args.unifusion_gee_sparse_weight),
            unifusion_gee_probe_weight=float(args.unifusion_gee_probe_weight),
            unifusion_gee_graph_weight=float(args.unifusion_gee_graph_weight),
            unifusion_gee_slot_weight=float(args.unifusion_gee_slot_weight),
            unifusion_gee_sibling_weight=float(args.unifusion_gee_sibling_weight),
            unifusion_gee_redundancy_weight=float(args.unifusion_gee_redundancy_weight),
            doc_id_to_idx=doc_id_to_idx,
            debug_trace=schemeb_debug_trace,
            unifusion_v9_enabled=bool(args.unifusion_v9),
            unifusion_v9_local_rerank=bool(args.unifusion_v9_local_rerank),
            unifusion_v9_dense_pool_k=int(args.unifusion_v9_dense_pool_k),
            unifusion_v9_sparse_pool_k=int(args.unifusion_v9_sparse_pool_k),
            unifusion_v9_candidate_pool_k=int(args.unifusion_v9_candidate_pool_k),
            unifusion_v9_graph_seed_k=int(args.unifusion_v9_graph_seed_k),
            unifusion_v9_graph_window=int(args.unifusion_v9_graph_window),
            unifusion_v9_preserve_top=int(args.unifusion_v9_preserve_top),
            unifusion_v9_base_weight=float(args.unifusion_v9_base_weight),
            unifusion_v9_dense_weight=float(args.unifusion_v9_dense_weight),
            unifusion_v9_sparse_weight=float(args.unifusion_v9_sparse_weight),
            unifusion_v9_probe_weight=float(args.unifusion_v9_probe_weight),
            unifusion_v9_graph_weight=float(args.unifusion_v9_graph_weight),
            unifusion_v9_slot_weight=float(args.unifusion_v9_slot_weight),
            unifusion_v9_diversity_weight=float(args.unifusion_v9_diversity_weight),
            unifusion_v9_modality_weight=float(args.unifusion_v9_modality_weight),
            unifusion_v10_conservative_rerank=bool(args.unifusion_v10_conservative_rerank),
            unifusion_v10_preserve_top=int(args.unifusion_v10_preserve_top),
            unifusion_v10_direct_preserve_top=int(args.unifusion_v10_direct_preserve_top),
            unifusion_v10_reference_pool_k=int(args.unifusion_v10_reference_pool_k),
            unifusion_v10_candidate_pool_k=int(args.unifusion_v10_candidate_pool_k),
            unifusion_v10_reference_weight=float(args.unifusion_v10_reference_weight),
            unifusion_v10_current_weight=float(args.unifusion_v10_current_weight),
            unifusion_v10_base_weight=float(args.unifusion_v10_base_weight),
            unifusion_v10_dense_weight=float(args.unifusion_v10_dense_weight),
            unifusion_v10_sparse_weight=float(args.unifusion_v10_sparse_weight),
            unifusion_v10_probe_weight=float(args.unifusion_v10_probe_weight),
            unifusion_v10_slot_weight=float(args.unifusion_v10_slot_weight),
            unifusion_v10_diversity_weight=float(args.unifusion_v10_diversity_weight),
            unifusion_v10_margin=float(args.unifusion_v10_margin),
            unifusion_v10_relevance_floor=float(args.unifusion_v10_relevance_floor),
            unifusion_source_evidence_fusion=bool(args.unifusion_source_evidence_fusion),
            unifusion_source_evidence_slot_verifier_bundle=unifusion_slot_verifier_bundle,
            unifusion_source_evidence_topk=int(args.unifusion_source_evidence_topk),
            unifusion_source_evidence_candidate_pool_k=int(args.unifusion_source_evidence_candidate_pool_k),
            unifusion_source_evidence_preserve_top=int(args.unifusion_source_evidence_preserve_top),
            unifusion_source_evidence_base_weight=float(args.unifusion_source_evidence_base_weight),
            unifusion_source_evidence_dense_weight=float(args.unifusion_source_evidence_dense_weight),
            unifusion_source_evidence_sparse_weight=float(args.unifusion_source_evidence_sparse_weight),
            unifusion_source_evidence_reference_weight=float(args.unifusion_source_evidence_reference_weight),
            unifusion_source_evidence_lexical_weight=float(args.unifusion_source_evidence_lexical_weight),
            unifusion_source_evidence_modality_prior_weight=float(args.unifusion_source_evidence_modality_prior_weight),
            unifusion_source_evidence_source_balance_weight=float(args.unifusion_source_evidence_source_balance_weight),
            unifusion_source_evidence_target_family_weight=float(args.unifusion_source_evidence_target_family_weight),
            unifusion_source_evidence_diversity_weight=float(args.unifusion_source_evidence_diversity_weight),
            unifusion_source_evidence_replacement_margin=float(args.unifusion_source_evidence_replacement_margin),
            unifusion_source_evidence_min_candidate_score=float(args.unifusion_source_evidence_min_candidate_score),
            unifusion_source_evidence_dense_guard=bool(args.unifusion_source_evidence_dense_guard),
            unifusion_source_evidence_dense_guard_topn=int(args.unifusion_source_evidence_dense_guard_topn),
            unifusion_source_evidence_dense_guard_prefixes=str(args.unifusion_source_evidence_dense_guard_prefixes),
            unifusion_source_evidence_dense_guard_weight=float(args.unifusion_source_evidence_dense_guard_weight),
            unifusion_source_evidence_dense_rank_weight=float(args.unifusion_source_evidence_dense_rank_weight),
            unifusion_source_evidence_current_rank_weight=float(args.unifusion_source_evidence_current_rank_weight),
            unifusion_source_evidence_source_balance_prefixes=str(args.unifusion_source_evidence_source_balance_prefixes),
            unifusion_source_evidence_max_changed_slots=int(args.unifusion_source_evidence_max_changed_slots),
            unifusion_source_evidence_slot_acceptance_guard=bool(args.unifusion_source_evidence_slot_acceptance_guard),
            unifusion_source_evidence_slot_acceptance_prefixes=str(args.unifusion_source_evidence_slot_acceptance_prefixes),
            unifusion_source_evidence_slot_acceptance_margin=float(args.unifusion_source_evidence_slot_acceptance_margin),
            unifusion_source_evidence_budget_composer=bool(args.unifusion_source_evidence_budget_composer),
            unifusion_source_evidence_budget_prefixes=str(args.unifusion_source_evidence_budget_prefixes),
            unifusion_source_evidence_budget_candidate_pool_k=int(args.unifusion_source_evidence_budget_candidate_pool_k),
            unifusion_source_evidence_budget_start_slot=int(args.unifusion_source_evidence_budget_start_slot),
            unifusion_source_evidence_budget_max_selected=int(args.unifusion_source_evidence_budget_max_selected),
            unifusion_source_evidence_budget_score_weight=float(args.unifusion_source_evidence_budget_score_weight),
            unifusion_source_evidence_budget_sibling_weight=float(args.unifusion_source_evidence_budget_sibling_weight),
            unifusion_source_evidence_budget_source_quota_weight=float(args.unifusion_source_evidence_budget_source_quota_weight),
            unifusion_source_evidence_budget_tail_rank_weight=float(args.unifusion_source_evidence_budget_tail_rank_weight),
            unifusion_source_evidence_budget_reference_weight=float(args.unifusion_source_evidence_budget_reference_weight),
            unifusion_source_evidence_budget_margin=float(args.unifusion_source_evidence_budget_margin),
            unifusion_source_evidence_budget_redundancy_weight=float(args.unifusion_source_evidence_budget_redundancy_weight),
            unifusion_source_evidence_sibling_filler=bool(args.unifusion_source_evidence_sibling_filler),
            unifusion_source_evidence_sibling_filler_prefixes=str(args.unifusion_source_evidence_sibling_filler_prefixes),
            unifusion_source_evidence_sibling_filler_candidate_pool_k=int(args.unifusion_source_evidence_sibling_filler_candidate_pool_k),
            unifusion_source_evidence_sibling_filler_start_slot=int(args.unifusion_source_evidence_sibling_filler_start_slot),
            unifusion_source_evidence_sibling_filler_max_selected=int(args.unifusion_source_evidence_sibling_filler_max_selected),
            unifusion_source_evidence_sibling_filler_tail_topn=int(args.unifusion_source_evidence_sibling_filler_tail_topn),
            unifusion_source_evidence_sibling_filler_reference_topn=int(args.unifusion_source_evidence_sibling_filler_reference_topn),
            unifusion_source_evidence_sibling_filler_margin=float(args.unifusion_source_evidence_sibling_filler_margin),
            unifusion_source_evidence_sibling_filler_sibling_weight=float(args.unifusion_source_evidence_sibling_filler_sibling_weight),
            unifusion_source_evidence_sibling_filler_reference_weight=float(args.unifusion_source_evidence_sibling_filler_reference_weight),
            unifusion_source_evidence_sibling_filler_tail_weight=float(args.unifusion_source_evidence_sibling_filler_tail_weight),
            unifusion_source_evidence_sibling_filler_dense_weight=float(args.unifusion_source_evidence_sibling_filler_dense_weight),
            unifusion_source_evidence_sibling_filler_source_weight=float(args.unifusion_source_evidence_sibling_filler_source_weight),
            unifusion_source_evidence_sibling_filler_redundancy_weight=float(args.unifusion_source_evidence_sibling_filler_redundancy_weight),
            unifusion_source_evidence_slot_verifier=bool(args.unifusion_source_evidence_slot_verifier),
            unifusion_source_evidence_slot_verifier_prefixes=str(args.unifusion_source_evidence_slot_verifier_prefixes),
            unifusion_source_evidence_slot_verifier_candidate_pool_k=int(args.unifusion_source_evidence_slot_verifier_candidate_pool_k),
            unifusion_source_evidence_slot_verifier_start_slot=int(args.unifusion_source_evidence_slot_verifier_start_slot),
            unifusion_source_evidence_slot_verifier_max_selected=int(args.unifusion_source_evidence_slot_verifier_max_selected),
            unifusion_source_evidence_slot_verifier_tail_topn=int(args.unifusion_source_evidence_slot_verifier_tail_topn),
            unifusion_source_evidence_slot_verifier_reference_topn=int(args.unifusion_source_evidence_slot_verifier_reference_topn),
            unifusion_source_evidence_slot_verifier_dense_topn=int(args.unifusion_source_evidence_slot_verifier_dense_topn),
            unifusion_source_evidence_slot_verifier_margin=float(args.unifusion_source_evidence_slot_verifier_margin),
            unifusion_source_evidence_slot_verifier_min_score=float(args.unifusion_source_evidence_slot_verifier_min_score),
            unifusion_source_evidence_slot_verifier_model_threshold=float(args.unifusion_source_evidence_slot_verifier_model_threshold),
            unifusion_source_evidence_slot_verifier_static_weight=float(args.unifusion_source_evidence_slot_verifier_static_weight),
            unifusion_source_evidence_slot_verifier_reference_weight=float(args.unifusion_source_evidence_slot_verifier_reference_weight),
            unifusion_source_evidence_slot_verifier_dense_weight=float(args.unifusion_source_evidence_slot_verifier_dense_weight),
            unifusion_source_evidence_slot_verifier_tail_weight=float(args.unifusion_source_evidence_slot_verifier_tail_weight),
            unifusion_source_evidence_slot_verifier_sibling_weight=float(args.unifusion_source_evidence_slot_verifier_sibling_weight),
            unifusion_source_evidence_slot_verifier_source_weight=float(args.unifusion_source_evidence_slot_verifier_source_weight),
            unifusion_source_evidence_slot_verifier_lexical_weight=float(args.unifusion_source_evidence_slot_verifier_lexical_weight),
            unifusion_source_evidence_slot_verifier_family_weight=float(args.unifusion_source_evidence_slot_verifier_family_weight),
            unifusion_source_evidence_slot_verifier_redundancy_weight=float(args.unifusion_source_evidence_slot_verifier_redundancy_weight),
            unifusion_source_evidence_kg_preservation_guard=bool(args.unifusion_source_evidence_kg_preservation_guard),
            unifusion_source_evidence_kg_preservation_prefixes=str(args.unifusion_source_evidence_kg_preservation_prefixes),
            unifusion_source_evidence_kg_preservation_min_kg=int(args.unifusion_source_evidence_kg_preservation_min_kg),
            unifusion_source_evidence_kg_preservation_candidate_pool_k=int(args.unifusion_source_evidence_kg_preservation_candidate_pool_k),
            unifusion_source_evidence_kg_preservation_start_slot=int(args.unifusion_source_evidence_kg_preservation_start_slot),
            unifusion_source_evidence_kg_preservation_margin=float(args.unifusion_source_evidence_kg_preservation_margin),
            unifusion_source_evidence_kg_preservation_reference_weight=float(args.unifusion_source_evidence_kg_preservation_reference_weight),
            unifusion_source_evidence_kg_preservation_dense_weight=float(args.unifusion_source_evidence_kg_preservation_dense_weight),
            unifusion_source_evidence_kg_preservation_current_weight=float(args.unifusion_source_evidence_kg_preservation_current_weight),
            unifusion_source_evidence_kg_preservation_family_weight=float(args.unifusion_source_evidence_kg_preservation_family_weight),
            unifusion_source_evidence_kg_preservation_lexical_weight=float(args.unifusion_source_evidence_kg_preservation_lexical_weight),
            unifusion_source_evidence_kg_verifier_bundle=unifusion_kg_verifier_bundle,
            unifusion_source_evidence_kg_verifier_weight=float(args.unifusion_source_evidence_kg_verifier_weight),
            unifusion_source_evidence_kg_verifier_min_score=float(args.unifusion_source_evidence_kg_verifier_min_score),
            unifusion_source_evidence_kg_verify_existing=bool(args.unifusion_source_evidence_kg_verify_existing),
            unifusion_source_evidence_kg_verify_existing_max_replacements=int(args.unifusion_source_evidence_kg_verify_existing_max_replacements),
            unifusion_source_budgeter_bundle=unifusion_source_budgeter_bundle,
            unifusion_source_budgeter_top1_guard=bool(args.unifusion_source_budgeter_top1_guard),
            unifusion_source_budgeter_need_threshold=float(args.unifusion_source_budgeter_need_threshold),
            unifusion_source_budgeter_non_kg_top1_max_kg=int(args.unifusion_source_budgeter_non_kg_top1_max_kg),
            unifusion_source_head_selector=bool(args.unifusion_source_head_selector),
            unifusion_source_head_topn=int(args.unifusion_source_head_topn),
            unifusion_source_head_source_weight=float(args.unifusion_source_head_source_weight),
            unifusion_source_head_same_query_weight=float(args.unifusion_source_head_same_query_weight),
            unifusion_source_head_position_weight=float(args.unifusion_source_head_position_weight),
            unifusion_source_head_reference_weight=float(args.unifusion_source_head_reference_weight),
            unifusion_source_head_lexical_weight=float(args.unifusion_source_head_lexical_weight),
            unifusion_source_head_base_weight=float(args.unifusion_source_head_base_weight),
            unifusion_source_head_dense_weight=float(args.unifusion_source_head_dense_weight),
            unifusion_source_head_sparse_weight=float(args.unifusion_source_head_sparse_weight),
            unifusion_source_head_margin=float(args.unifusion_source_head_margin),
            unifusion_source_head_off_source_margin=float(args.unifusion_source_head_off_source_margin),
            unifusion_source_action_policy_bundle=unifusion_source_action_policy_bundle,
            unifusion_source_action_policy_min_prob=float(args.unifusion_source_action_policy_min_prob),
            unifusion_source_action_policy_topk=int(args.unifusion_source_action_policy_topk),
            unifusion_source_action_policy_pool_k=int(args.unifusion_source_action_policy_pool_k),
            unifusion_final_evidence_composer=bool(args.unifusion_final_evidence_composer),
            unifusion_final_evidence_topk=int(args.unifusion_final_evidence_topk),
            unifusion_final_evidence_candidate_pool_k=int(args.unifusion_final_evidence_candidate_pool_k),
            unifusion_final_evidence_dense_pool_k=int(args.unifusion_final_evidence_dense_pool_k),
            unifusion_final_evidence_sparse_pool_k=int(args.unifusion_final_evidence_sparse_pool_k),
            unifusion_final_evidence_preserve_top=int(args.unifusion_final_evidence_preserve_top),
            unifusion_final_evidence_max_replacements=int(args.unifusion_final_evidence_max_replacements),
            unifusion_final_evidence_min_candidate_score=float(args.unifusion_final_evidence_min_candidate_score),
            unifusion_final_evidence_replacement_margin=float(args.unifusion_final_evidence_replacement_margin),
            unifusion_final_evidence_min_query_overlap=float(args.unifusion_final_evidence_min_query_overlap),
            unifusion_final_evidence_source_need_weight=float(args.unifusion_final_evidence_source_need_weight),
            unifusion_final_evidence_redundancy_weight=float(args.unifusion_final_evidence_redundancy_weight),
            unifusion_final_evidence_verifier_bundle=unifusion_final_evidence_verifier_bundle,
            unifusion_final_evidence_verifier_threshold=float(args.unifusion_final_evidence_verifier_threshold),
            unifusion_final_evidence_verifier_margin=float(args.unifusion_final_evidence_verifier_margin),
        )
        ...
        if "dense_concat" in rankings and "unifusion_rag" in rankings:
            dense10 = rankings["dense_concat"][:10]
            uni10 = rankings["unifusion_rag"][:10]
            if dense10 == uni10:
        ...
        for m in selected_methods:
            ...
            top10_ids = [doc_ids[j] for j in idxs[:10]]
            per_method_top10[m].append(top10_ids)
            if m == "unifusion_submod" and submodular_t2g_packer is not None:
            ...
            elif m in {"unifusion_rag", "unifusion_submod", "ablation_no_redundancy_e2e", "ablation_no_pathmaxsim_e2e"}:
            ...
            if m == "unifusion_rag" and bool(args.unifusion_policy_context):
                if select_unifusion_policy_context is None or UnifusionPolicyConfig is None:
                    raise RuntimeError("unifusion_policy module is required for --unifusion-policy-context")
                policy_cfg = UnifusionPolicyConfig(
                    candidate_pool_k=int(args.unifusion_policy_pool_k),
                    dense_pool_k=int(args.unifusion_policy_dense_pool_k),
                    target_weight=float(args.unifusion_policy_target_weight),
                    coverage_weight=float(args.unifusion_policy_coverage_weight),
                    diversity_weight=float(args.unifusion_policy_diversity_weight),
                )
                ctx_idxs, policy_trace = select_unifusion_policy_context(
                    query=q_texts[i],
                ...
                context_debug_trace.setdefault("unifusion_policy_replaced_count", []).append(float(policy_trace.replaced_count))
                context_debug_trace.setdefault("unifusion_policy_forced_hits", []).append(float(policy_trace.forced_hits))
                context_debug_trace.setdefault("unifusion_policy_coverage", []).append(float(policy_trace.coverage))
            ctx = [doc_texts[j] for j in ctx_idxs]
            ctx_doc_ids = [doc_ids[j] for j in ctx_idxs]
            pred = reader_fn(q_texts[i], ctx)
            if (
                args.enable_unifusion_answer_type_guard
                and is_llm_reader
                and m in {"unifusion_rag", "unifusion_submod", "ablation_no_redundancy_e2e", "ablation_no_pathmaxsim_e2e"}
            ):
                pred = apply_unifusion_answer_type_guard(
                    q_texts[i],
                    pred,
                    ctx,
            ...
            if (
                args.enable_unifusion_answer_calibration
                and is_llm_reader
                and m in {"unifusion_rag", "unifusion_submod", "ablation_no_redundancy_e2e", "ablation_no_pathmaxsim_e2e"}
            ):
                pred = calibrate_unifusion_answer(
                    q_texts[i],
                    pred,
                    ctx,
                    extractive_numeric_consensus=bool(args.extractive_numeric_consensus),
            ...
            if (
                bool(args.unifusion_no_evidence_retry)
                and is_llm_reader
                and m in {"unifusion_rag", "unifusion_submod", "ablation_no_redundancy_e2e", "ablation_no_pathmaxsim_e2e"}
            ):
                if (
                    is_no_evidence_answer is None
                    or select_unifusion_retry_context is None
                    or UnifusionRetryAgentConfig is None
                ):
                    raise RuntimeError("unifusion_policy module is required for --unifusion-no-evidence-retry")
                base_answer_support0 = answer_support_score(pred, ctx)
                no_evidence_answer = bool(is_no_evidence_answer(pred))
                low_support_answer = (
                    float(args.unifusion_no_evidence_retry_low_support_threshold) >= 0.0
                    and base_answer_support0 < float(args.unifusion_no_evidence_retry_low_support_threshold)
                )
                context_debug_trace.setdefault("unifusion_no_evidence_retry_no_evidence", []).append(
                    float(no_evidence_answer)
                )
                context_debug_trace.setdefault("unifusion_no_evidence_retry_base_support", []).append(
                ...
                if no_evidence_answer or low_support_answer:
                    ...
                    retry_cfg = UnifusionRetryAgentConfig(
                        candidate_pool_k=int(args.unifusion_no_evidence_retry_pool_k),
                        dense_pool_k=int(args.unifusion_no_evidence_retry_dense_pool_k),
                    )
                    retry_k = max(
                        int(effective_context_k_i),
                        int(args.unifusion_no_evidence_retry_context_k),
                    )
                    retry_ctx_idxs, retry_agent_trace = select_unifusion_retry_context(
                        query=q_texts[i],
                        current_answer=pred,
                        base_ctx_idxs=ctx_idxs,
                        ranked_idxs=idxs,
                    ...
                    context_debug_trace.setdefault("unifusion_no_evidence_retry_context_k", []).append(
                        float(len(retry_ctx_idxs))
                    )
                    context_debug_trace.setdefault("unifusion_no_evidence_retry_coverage", []).append(
                    ...
                    if retry_ctx_idxs and retry_ctx_idxs != ctx_idxs:
                        ...
                        if args.enable_unifusion_answer_type_guard and is_llm_reader:
                            retry_pred = apply_unifusion_answer_type_guard(
                                q_texts[i],
                                retry_pred,
                                retry_ctx,
                                extractive_numeric_consensus=bool(args.extractive_numeric_consensus),
                            )
                        if args.enable_unifusion_answer_calibration and is_llm_reader:
                            retry_pred = calibrate_unifusion_answer(
                                q_texts[i],
                                retry_pred,
                                retry_ctx,
                                extractive_numeric_consensus=bool(args.extractive_numeric_consensus),
                        ...
                        if no_evidence_answer:
                            accept_retry0 = (
                                ...
                                and retry_answer_support0 >= float(args.unifusion_no_evidence_retry_min_support)
                            )
                        if low_support_answer:
                            accept_retry0 = accept_retry0 or (
                                not retry_no_evidence
                                and retry_answer_support0 >= base_answer_support0 + float(args.unifusion_no_evidence_retry_margin)
                            )
                        context_debug_trace.setdefault("unifusion_no_evidence_retry_retry_support", []).append(
                            float(retry_answer_support0)
                        )
                        context_debug_trace.setdefault("unifusion_no_evidence_retry_accept", []).append(
                            float(accept_retry0)
                        )
                        if accept_retry0:
                            pred = retry_pred
            ...
            if (
                bool(args.unifusion_table_number_agent)
                and is_llm_reader
                and m in {"unifusion_rag", "unifusion_submod", "ablation_no_redundancy_e2e", "ablation_no_pathmaxsim_e2e"}
            ):
                pred, table_number_applied, table_number_debug = maybe_apply_unifusion_table_number_agent(
                    q_texts[i],
                    pred,
                    ctx,
                    ctx_doc_ids,
                ...
                if table_number_debug.get("attempted", 0.0) > 0.0:
                    ...
                    for tn_key, tn_value in table_number_debug.items():
                        context_debug_trace.setdefault(f"unifusion_table_number_agent_{tn_key}", []).append(
                            float(tn_value)
                        )
            ...
            retry_controls = None
            if verifier_bundle is not None and is_llm_reader and m in {"unifusion_rag", "unifusion_submod", "ablation_no_redundancy_e2e", "ablation_no_pathmaxsim_e2e"}:
                ...
                retry_controls = verifier_bundle.derive_retry_controls(
                    ...
                    doc_ids=ctx_doc_ids,
                    router_entropy=float(router_entropy[i]),
                    base_support_retry_threshold=float(args.unifusion_support_retry_threshold),
                    base_support_retry_margin=float(args.unifusion_support_retry_margin),
                    base_support_retry_mode=str(args.unifusion_support_retry_mode),
                    base_consensus_support_threshold=float(args.unifusion_consensus_refine_support_threshold),
                    conflict_probability=conflict_probability,
                )
                controller_verifier_support.append(float(verifier_support_prob))

            if (
                float(args.unifusion_support_retry_threshold) >= 0.0
                and is_llm_reader
                and m in {"unifusion_rag", "unifusion_submod", "ablation_no_redundancy_e2e", "ablation_no_pathmaxsim_e2e"}
            ):
                ...
                support_retry_threshold_eff = float(args.unifusion_support_retry_threshold)
                support_retry_margin_eff = float(args.unifusion_support_retry_margin)
                support_retry_mode_eff = str(args.unifusion_support_retry_mode)
                ...
                if bool(args.unifusion_support_retry_targeted_only):
                    if target_type not in {"number", "year", "location"}:
                        allow_retry = False
                if allow_retry and int(args.unifusion_support_retry_complexity_min) > 0:
                    if int(query_complexity_level) < int(args.unifusion_support_retry_complexity_min):
                        allow_retry = False
                if allow_retry and float(args.unifusion_support_retry_entropy_min) >= 0.0:
                    if float(router_entropy[i]) < float(args.unifusion_support_retry_entropy_min):
                        allow_retry = False
                    ...
                    if base_support < support_retry_threshold_eff:
                        ...
                        if retry_mode == "evidence_chain":
                            retry_ctx_idxs = build_evidence_chain_retry_context(
                                ...
                                doc_tokens=doc_tokens,
                                k=effective_context_k_i,
                                pool_k=int(args.unifusion_support_retry_pool_k),
                                answer_boost=float(args.unifusion_support_retry_answer_boost),
                                target_type=target_type,
                            )
                        else:
            ...
            if (
                args.enable_unifusion_consensus_refine
                and is_llm_reader
                and m in {"unifusion_rag", "unifusion_submod", "ablation_no_redundancy_e2e", "ablation_no_pathmaxsim_e2e"}
            ):
                allow_refine = True
                consensus_support_threshold_eff = float(args.unifusion_consensus_refine_support_threshold)
                if retry_controls is not None:
                    consensus_support_threshold_eff = float(retry_controls["consensus_support_threshold"])
                if int(args.unifusion_consensus_refine_complexity_min) > 0:
                    if int(query_complexity_level) < int(args.unifusion_consensus_refine_complexity_min):
                        allow_refine = False
                if allow_refine and float(args.unifusion_consensus_refine_entropy_min) >= 0.0:
                    if float(router_entropy[i]) < float(args.unifusion_consensus_refine_entropy_min):
                        allow_refine = False
                ...
                if allow_refine:
                    pred, refine_attempted, refine_applied = refine_unifusion_answer_with_consensus(
                        q_texts[i],
                        pred,
                        ctx,
                        extractive_numeric_consensus=bool(args.extractive_numeric_consensus),
                        min_gain=float(args.unifusion_consensus_refine_min_gain),
                        targeted_only=bool(args.unifusion_consensus_refine_targeted_only),
                    )
                    if refine_attempted:
    ...
    results = {
        "meta": {
            ...
            "preserve_dense_top": args.preserve_dense_top,
            "unifusion_late_alpha": args.unifusion_late_alpha,
            "unifusion_v9": bool(args.unifusion_v9),
            "unifusion_v9_local_rerank": bool(args.unifusion_v9_local_rerank),
            "unifusion_v9_dense_pool_k": int(args.unifusion_v9_dense_pool_k),
            "unifusion_v9_sparse_pool_k": int(args.unifusion_v9_sparse_pool_k),
            "unifusion_v9_candidate_pool_k": int(args.unifusion_v9_candidate_pool_k),
            ...
            "unifusion_source_evidence_slot_verifier_margin": float(args.unifusion_source_evidence_slot_verifier_margin),
            "unifusion_source_evidence_slot_verifier_min_score": float(args.unifusion_source_evidence_slot_verifier_min_score),
            "unifusion_source_evidence_slot_verifier_model": str(args.unifusion_source_evidence_slot_verifier_model)
            if args.unifusion_source_evidence_slot_verifier_model is not None
            else None,
            "unifusion_source_evidence_slot_verifier_model_loaded": bool(unifusion_slot_verifier_bundle is not None),
            "unifusion_source_evidence_slot_verifier_model_threshold": float(args.unifusion_source_evidence_slot_verifier_model_threshold),
            "unifusion_source_evidence_slot_verifier_static_weight": float(args.unifusion_source_evidence_slot_verifier_static_weight),
            "unifusion_source_evidence_slot_verifier_reference_weight": float(args.unifusion_source_evidence_slot_verifier_reference_weight),
            "unifusion_source_evidence_slot_verifier_dense_weight": float(args.unifusion_source_evidence_slot_verifier_dense_weight),
            ...
            "unifusion_source_evidence_kg_preservation_family_weight": float(args.unifusion_source_evidence_kg_preservation_family_weight),
            "unifusion_source_evidence_kg_preservation_lexical_weight": float(args.unifusion_source_evidence_kg_preservation_lexical_weight),
            "unifusion_source_evidence_kg_verifier_model": str(args.unifusion_source_evidence_kg_verifier_model)
            if args.unifusion_source_evidence_kg_verifier_model is not None
            else None,
            "unifusion_source_evidence_kg_verifier_model_loaded": bool(unifusion_kg_verifier_bundle is not None),
            "unifusion_source_evidence_kg_verifier_meta_summary": unifusion_kg_verifier_meta_summary,
            "unifusion_source_evidence_kg_verifier_weight": float(args.unifusion_source_evidence_kg_verifier_weight),
            "unifusion_source_evidence_kg_verifier_min_score": float(args.unifusion_source_evidence_kg_verifier_min_score),
            "unifusion_source_evidence_kg_verify_existing": bool(args.unifusion_source_evidence_kg_verify_existing),
            "unifusion_source_evidence_kg_verify_existing_max_replacements": int(
                args.unifusion_source_evidence_kg_verify_existing_max_replacements
            ),
            "unifusion_source_budgeter_model": str(args.unifusion_source_budgeter_model)
            if args.unifusion_source_budgeter_model is not None
            else None,
            "unifusion_source_budgeter_model_loaded": bool(unifusion_source_budgeter_bundle is not None),
            "unifusion_source_budgeter_meta_summary": unifusion_source_budgeter_meta_summary,
            "unifusion_source_budgeter_top1_guard": bool(args.unifusion_source_budgeter_top1_guard),
            "unifusion_source_budgeter_need_threshold": float(args.unifusion_source_budgeter_need_threshold),
            "unifusion_source_budgeter_non_kg_top1_max_kg": int(args.unifusion_source_budgeter_non_kg_top1_max_kg),
            "unifusion_source_head_selector": bool(args.unifusion_source_head_selector),
            ...
            "unifusion_source_head_margin": float(args.unifusion_source_head_margin),
            "unifusion_source_head_off_source_margin": float(args.unifusion_source_head_off_source_margin),
            "unifusion_source_action_policy_model": str(args.unifusion_source_action_policy_model)
            if args.unifusion_source_action_policy_model is not None
            else None,
            "unifusion_source_action_policy_model_loaded": bool(unifusion_source_action_policy_bundle is not None),
            "unifusion_source_action_policy_meta_summary": unifusion_source_action_policy_meta_summary,
            "unifusion_source_action_policy_min_prob": float(args.unifusion_source_action_policy_min_prob),
            "unifusion_source_action_policy_topk": int(args.unifusion_source_action_policy_topk),
            "unifusion_source_action_policy_pool_k": int(args.unifusion_source_action_policy_pool_k),
            "unifusion_final_evidence_composer": bool(args.unifusion_final_evidence_composer),
            ...
            "unifusion_final_evidence_source_need_weight": float(args.unifusion_final_evidence_source_need_weight),
            "unifusion_final_evidence_redundancy_weight": float(args.unifusion_final_evidence_redundancy_weight),
            "unifusion_final_evidence_verifier_model": str(args.unifusion_final_evidence_verifier_model)
            if args.unifusion_final_evidence_verifier_model is not None
            else None,
            "unifusion_final_evidence_verifier_model_loaded": bool(
                unifusion_final_evidence_verifier_bundle is not None
            ),
            "unifusion_final_evidence_verifier_meta_summary": unifusion_final_evidence_verifier_meta_summary,
            "unifusion_final_evidence_verifier_threshold": float(args.unifusion_final_evidence_verifier_threshold),
            "unifusion_final_evidence_verifier_margin": float(args.unifusion_final_evidence_verifier_margin),
            ...
            "unifusion_moe_sibling_window": int(args.unifusion_moe_sibling_window),
            "unifusion_moe_sibling_weight": float(args.unifusion_moe_sibling_weight),
            "unifusion_ser_ranker": str(args.unifusion_ser_ranker) if args.unifusion_ser_ranker is not None else None,
            "unifusion_ser_loaded": bool(unifusion_ser_bundle is not None),
            "unifusion_ser_meta_summary": unifusion_ser_meta_summary,
            "unifusion_ser_candidate_pool_k": int(args.unifusion_ser_candidate_pool_k),
            "unifusion_ser_dense_pool_k": int(args.unifusion_ser_dense_pool_k),
            "unifusion_ser_sparse_pool_k": int(args.unifusion_ser_sparse_pool_k),
            "unifusion_ser_preserve_top": int(args.unifusion_ser_preserve_top),
            ...
            "unifusion_gee_sibling_weight": float(args.unifusion_gee_sibling_weight),
            "unifusion_gee_redundancy_weight": float(args.unifusion_gee_redundancy_weight),
            "unifusion_policy_context": bool(args.unifusion_policy_context),
            "unifusion_policy_pool_k": int(args.unifusion_policy_pool_k),
            "unifusion_policy_dense_pool_k": int(args.unifusion_policy_dense_pool_k),
            "unifusion_policy_target_weight": float(args.unifusion_policy_target_weight),
            "unifusion_policy_coverage_weight": float(args.unifusion_policy_coverage_weight),
            "unifusion_policy_diversity_weight": float(args.unifusion_policy_diversity_weight),
            "unifusion_no_evidence_retry": bool(args.unifusion_no_evidence_retry),
            "unifusion_no_evidence_retry_context_k": int(args.unifusion_no_evidence_retry_context_k),
            "unifusion_no_evidence_retry_pool_k": int(args.unifusion_no_evidence_retry_pool_k),
            "unifusion_no_evidence_retry_dense_pool_k": int(args.unifusion_no_evidence_retry_dense_pool_k),
            "unifusion_no_evidence_retry_min_support": float(args.unifusion_no_evidence_retry_min_support),
            "unifusion_no_evidence_retry_margin": float(args.unifusion_no_evidence_retry_margin),
            "unifusion_no_evidence_retry_low_support_threshold": float(args.unifusion_no_evidence_retry_low_support_threshold),
            "unifusion_no_evidence_retry_attempted": int(no_evidence_retry_attempted),
            "unifusion_no_evidence_retry_applied": int(no_evidence_retry_applied),
            "unifusion_table_number_agent": bool(args.unifusion_table_number_agent),
            "unifusion_table_number_agent_min_score": float(args.unifusion_table_number_agent_min_score),
            "unifusion_table_number_agent_min_support": float(args.unifusion_table_number_agent_min_support),
            "unifusion_table_number_agent_margin": float(args.unifusion_table_number_agent_margin),
            "unifusion_table_number_agent_low_support_threshold": float(
                args.unifusion_table_number_agent_low_support_threshold
            ),
            "unifusion_table_number_agent_attempted": int(table_number_agent_attempted),
            "unifusion_table_number_agent_applied": int(table_number_agent_applied),
            "context_upo_lite_quota_mix": float(args.context_upo_lite_quota_mix),
            "context_upo_lite_rerank_bonus": float(args.context_upo_lite_rerank_bonus),
            ...
            "enable_unifusion_answer_type_guard": bool(args.enable_unifusion_answer_type_guard),
            "enable_unifusion_answer_calibration": bool(args.enable_unifusion_answer_calibration),
            "unifusion_support_retry_threshold": float(args.unifusion_support_retry_threshold),
            "unifusion_support_retry_margin": float(args.unifusion_support_retry_margin),
            "unifusion_support_retry_targeted_only": bool(args.unifusion_support_retry_targeted_only),
            "unifusion_support_retry_mode": str(args.unifusion_support_retry_mode),
            "unifusion_support_retry_pool_k": int(args.unifusion_support_retry_pool_k),
            "unifusion_support_retry_answer_boost": float(args.unifusion_support_retry_answer_boost),
            "unifusion_support_retry_complexity_min": int(args.unifusion_support_retry_complexity_min),
            "unifusion_support_retry_entropy_min": float(args.unifusion_support_retry_entropy_min),
            "unifusion_support_retry_attempted": int(support_retry_attempted),
            "unifusion_support_retry_applied": int(support_retry_applied),
            "enable_unifusion_consensus_refine": bool(args.enable_unifusion_consensus_refine),
            "unifusion_consensus_refine_min_gain": float(args.unifusion_consensus_refine_min_gain),
            "unifusion_consensus_refine_support_threshold": float(args.unifusion_consensus_refine_support_threshold),
            "unifusion_consensus_refine_complexity_min": int(args.unifusion_consensus_refine_complexity_min),
            "unifusion_consensus_refine_entropy_min": float(args.unifusion_consensus_refine_entropy_min),
            "unifusion_consensus_refine_targeted_only": bool(args.unifusion_consensus_refine_targeted_only),
            "unifusion_consensus_refine_attempted": int(consensus_refine_attempted),
            "unifusion_consensus_refine_applied": int(consensus_refine_applied),
            "dense_vs_unifusion_same_top10_ratio": float(dense_uni_same_top10 / max(1, len(rows))),
            "dense_vs_unifusion_avg_top10_overlap": float(dense_uni_overlap_sum / max(1, len(rows))),
    ...
    if results["meta"]["dense_vs_unifusion_same_top10_ratio"] >= 0.95:
        print(
            "[warn] Dense vs UniFusion top-10 rankings are almost identical. "
