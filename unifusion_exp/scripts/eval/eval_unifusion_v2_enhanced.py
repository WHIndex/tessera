#!/usr/bin/env python3
"""
UniFusion-RAG V2 Enhanced: 真正集成 Router + KG + Late Interaction 的检索评测

注意：该脚本为历史实验脚本（experimental），不作为论文主结果口径。
论文主线请使用 scripts/eval/run_e2e_table1c.py 与 scripts/eval/eval_unifusion_retrieval_main.py。
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate UniFusion V2 Enhanced")
    parser.add_argument(
        "--experimental-ok",
        action="store_true",
    ...
    if not args.experimental_ok:
        raise SystemExit(
            "Blocked: eval_unifusion_v2_enhanced.py is experimental and not paper-mainline. "
            "Re-run with --experimental-ok only for exploratory checks."
        )

    print("=" * 60)
    print("UniFusion-RAG V2 Enhanced Evaluation")
    print("=" * 60)
    ...
    print("[Eval] Building enhanced UniFusion predictions...")
    ...
    methods = {
        ...
        "unifusion_v2_enhanced": pred_main,
    }
    ...
    out = {
        ...
        "methods": metrics,
        "main_method": "unifusion_v2_enhanced",
        "diagnostics": {
