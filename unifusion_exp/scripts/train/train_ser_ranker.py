...
from unifusion_exp.e2e.graph_evidence import GraphEvidenceConfig, expand_graph_evidence_candidates  # noqa: E402
from unifusion_exp.e2e.objectives import infer_query_modality_prior, infer_qa_target_type  # noqa: E402
from unifusion_exp.e2e.ser_ranker import (  # noqa: E402
    SERRankerBundle,
    SERRankerConfig,
    SER_FEATURE_NAMES,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train UniFusion-SER supervised evidence-set reranker")
        ...
        for label, path in bad_paths:
            if is_test_like_path(Path(path)):
                raise ValueError(
                    ...
                    "Set UNIFUSION_ALLOW_TEST_SPLIT_TRAINING=1 only for debugging, never for paper results."
                )
