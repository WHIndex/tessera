...
from unifusion_exp.e2e.pairwise_slot_verifier import PESV_FEATURE_NAMES, PairwiseSlotVerifierBundle, save_pairwise_slot_verifier_bundle  # noqa: E402
    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            if not line.strip():
                continue
            row = json.loads(line)
            ranking = list((row.get("rankings") or {}).get("unifusion_rag") or [])
...
def main() -> int:
    ...
    ap.add_argument("--run-id", default="unifusion_pesv_v1")
